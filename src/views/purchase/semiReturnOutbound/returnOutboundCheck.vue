<template>
  <el-row v-loading="$store.getters.tb_loading">
    <div class="panel">
      <div class="panel-hd">
        <span class="title">查看退货出库单({{detail.KindTypeEv}})</span>
      </div>
      <div class="panel-bd">
              ...
              <tr>
                <td rowspan="4" class="state-img">
                  <img
                    ...
                    v-if="detail.State === HalfReturnOrderBasicState.Draft"
                  >
                  <img
                    src="@/assets/images/auditing.png"
                    v-if="detail.State === HalfReturnOrderBasicState.Wait"
                  >
                  <img
                    src="@/assets/images/audited.png"
                    v-if="detail.State === HalfReturnOrderBasicState.Audit"
                  >
                  <img
                    src="@/assets/images/auditBack.png"
                    v-if="detail.State === HalfReturnOrderBasicState.Reject"
                  >
                  <img
                    src="@/assets/images/abandon.png"
                    v-if="detail.State === HalfReturnOrderBasicState.Abandon || detail.State === HalfReturnOrderBasicState.Cancel"
                  >
                  <div>{{HalfReturnOrderBasicState.Types[detail.State]}}</div>
                </td>
              </tr>
              <tr>
                <td class="tit">单号</td>
        ...
        <div class="p-x-10">
          <el-table :data="data" class="m-b-10" element-loading-text="拼命加载中">
            <el-table-column label="序号" type="index" width="50"></el-table-column>
            <el-table-column prop="HalfName" label="半成色名称" min-width="80" show-overflow-tooltip></el-table-column>
    ...
    <el-row style="margin-top: 10px; text-align: left; border: 0;">
      <el-button
        name="btnEdit"
        type="primary"
        @click="$router.push({path:'/purchase/semiReturnOutbound/returnOutboundEdit',query:{id: detail.ReturnId}})"
        v-if="detail.State == HalfReturnOrderBasicState.Draft || detail.State == HalfReturnOrderBasicState.Reject"
      >编辑</el-button>
      <el-button
        name="btnAudit"
        @click="showAudit"
...
<script>
import { HalfReturnOrderBasicState } from '@/enums/stocking'
...
import {
  ...
  STOCKING_API_HALF_RETURN_ORDER_ITEM_EXPORTGETSHALF
} from '@/apis/stocking'
