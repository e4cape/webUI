<template>
  <el-row>
    <div class="panel" v-loading="$store.getters.tb_loading">
      <div class="panel-hd">
        <span class="title">编辑退货出库单({{StuffType.Types[detail.StuffType]}})</span>
      ...
      <div class="panel-bd">
        ...
        <div class="p-x-10">
          ...
          >
            ...
            <el-table-column prop="StoneClassTypeEk" label="石类" min-width="120">
              ...
              <template slot-scope="scope">
                <el-select
                  ...
                  @change="handleFocus(scope.row, $store.getters.stone)"
                >
                  <el-option
            ...
            <el-table-column prop="StoneWeightEk" label="石重范围(ct)" min-width="120">
              ...
              >
                <el-select
                  ...
                  @change="handleFocus(scope.row, StoneWeight)"
                >
            ...
            <el-table-column prop="StoneCut" label="切工" min-width="100">
              <template
                slot-scope="scope"
                v-if="scope.$index >= currentStartIndex && scope.$index <= currentEndIndex"
              >
            ...
            <el-table-column prop="PartTypeEk" label="配件名称" min-width="80">
              ...
              <template slot-scope="scope">
                <el-select
                  ...
                  @change="handleFocus(scope.row, PartType)"
                >
                  <el-option
    ...
    <template-import
      ...
      :option="importOption"
      :root="$root.filePaths.STOCKING_PURCHASE + '/stuffReturn/'"
    ></template-import>
    <!-- End 导入货品 -->
    <!-- End 智能导入货品 -->
    <smart-import
...
import {
  ...
  STOCKING_API_STUFF_RETURN_ORDER_BASIC_UPDATE,
  STOCKING_API_STUFF_RETURN_ORDER_ITEM_UPDATE,
  STOCKING_API_STUFF_RETURN_ORDER_ITEM_CREATE,
  STOCKING_API_STUFF_RETURN_ORDER_BASIC_WAIT,
  STOCKING_API_STUFF_RETURN_ORDER_ITEM_DELETE
} from '@/apis/stocking'
import {
...
export default {
  ...
  methods: {
    ...
    showEdit() {
      ...
      this.detail.ReasonTypeDk =
        this.detail.ReasonTypeDk === 0 ? '' : this.detail.ReasonTypeDk
        ...
        if (this.data[i].isEdit || this.data[i].isError) {
          ...
          if (this.data[i].isEdit) {
            ...
            if (this.data[i].ItemId) {
              res = await STOCKING_API_STUFF_RETURN_ORDER_ITEM_UPDATE(param)
            } else {
              res = await STOCKING_API_STUFF_RETURN_ORDER_ITEM_CREATE(param)
            }
    ...
    submit() {
      STOCKING_API_STUFF_RETURN_ORDER_ITEM_GETS(this.parameters).then(res => {
        if (res.data.Code === 'CORRECT') {
          ...
          if (data.length === 0) {
            return this.$message.error('请添加需要退货的货品')
          } else {
            STOCKING_API_STUFF_RETURN_ORDER_BASIC_WAIT({
              ReturnId: this.parameters.ReturnId
            }).then(res => {
              if (res.data.Code === 'CORRECT') {
                this.$router.push('/purchase/stockRemoval/index')
              }
            })
          }
        }
