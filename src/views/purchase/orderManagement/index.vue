<template>
  <el-row class="content">
    <el-form :model="queryForm" ref="search" lable-width="120px" class="item-lh-26" :inline="true">
      <search-panel @onSearch="onSearch" @onReset="onReset">
        <template slot="btnBox">
          <el-form-item>
            <el-button type="primary" :disabled="newKind.length == 0 ? true : false" v-if="newKind.length <= 1" @click="openNewOrderForm(newKind[0].Id)">新建</el-button>
            <el-dropdown @command="openNewOrderForm" type="primary" v-if="newKind.length > 1">
              <el-button name="btnmultiCheckPrint" type="primary">
                新建
                <i class="el-icon-caret-bottom el-icon--right"></i>
              </el-button>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-for="(item,index) in newKind" :key="index" :command="item.Id" style="min-width: 80px; text-align: center;">{{item.Value}}</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </el-form-item>
        </template>
        <template slot="simpleSearch">
          <el-form-item prop="State">
            <el-select filterable name="State" v-model="queryForm.State" placeholder="所有">
              ...
              <el-option v-for="(item,index) in orderBasicState.TypeArray" :key="index" :label="item.Value" :value="String(item.KeyId)"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item prop="PurchaseCode">
            <el-input name="PurchaseCode" v-model="queryForm.PurchaseCode" :maxlength="50" @keyup.enter.native="onSearch" placeholder="单据编号">
        ...
        <template slot="seniorSearch">
          ...
          <el-form-item label="所有方式：" prop="PurchaseType">
            <el-select filterable name="PurchaseType" v-model="queryForm.PurchaseType" placeholder="所有方式">
              ...
              <el-option :label="purchaseType.Types[purchaseType.Purchase]" :value="String(purchaseType.Purchase)"></el-option>
              <el-option :label="purchaseType.Types[purchaseType.WeiwStuff]" :value="String(purchaseType.WeiwStuff)"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item prop="PurchaseCode" label="单据编号：">
            <el-input name="PurchaseCode" v-model="queryForm.PurchaseCode" :maxlength="50" @keyup.enter.native="onSearch"></el-input>
          </el-form-item>
    ...
    <!-- Data Table -->
    <el-table :data="data" element-loading-text="拼命加载中" @sort-change="querySort" v-loading="$store.getters.tb_loading">
      <el-table-column type="selection" fixed></el-table-column>
      <el-table-column prop="PurchaseCode" fixed label="单据编号" min-width="140" show-overflow-tooltip sortable="custom"></el-table-column>
      ...
      <el-table-column label="业务日期" min-width="100" show-overflow-tooltip sortable="custom">
      ...
      <el-table-column prop="ItemQty" label="订单数量" min-width="100" show-overflow-tooltip></el-table-column>
      <el-table-column prop="ArriveQty" label="到货数量" min-width="100" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column label="预计到货日期" min-width="100" show-overflow-tooltip>
        <template slot-scope="scope">{{scope.row.ForwdDate | filterDate }}</template>
      </el-table-column>
      <el-table-column label="最后操作时间" min-width="100" show-overflow-tooltip>
        <template slot-scope="scope" v-if="scope.row.State !== orderBasicState.Draft">{{scope.row.CheckTime | filterDateTime }}</template>
      </el-table-column>
      <el-table-column label="状态" min-width="80" show-overflow-tooltip fixed="right">
        <template slot-scope="scope">
          <el-tooltip
            placement="top"
            v-if="(scope.row.State === orderBasicState.Reject || scope.row.State === orderBasicState.Abandon || scope.row.State === orderBasicState.Cancel) && scope.row.CheckNote"
          >
            <div slot="content">{{scope.row.CheckNote}}</div>
            <span :class="scope.row.State | findKey(orderBasicState)">{{orderBasicState.Types[scope.row.State] || '-'}}</span>
          </el-tooltip>
          <span :class="scope.row.State | findKey(orderBasicState)" v-else>{{orderBasicState.Types[scope.row.State] || '-'}}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" min-width="200" fixed="right">
        <template slot-scope="scope">
          <el-button name="btnViewData" type="text" @click="viewOrderForm(scope.row.PurchaseId)">查看</el-button>
          <el-button v-if="scope.row.State == orderBasicState.Draft || scope.row.State == orderBasicState.Reject" name="editData" type="text" @click="editOrderForm(scope.row.PurchaseId)">编辑</el-button>
          <el-button v-if="scope.row.State == orderBasicState.Wait" name="btnWaitData" type="text" @click="reviewStyleOrder(scope.row)">审核</el-button>
          <el-button v-if="scope.row.State == orderBasicState.Draft || scope.row.State == orderBasicState.Reject" name="btnShowDetail" type="text" @click="openObsolete(scope.row)">作废</el-button>
          <el-button v-if="scope.row.State == orderBasicState.Audit" name="btncancelAudit" type="text" @click="openCancelReview(scope.row)">取消审核</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- Pagination -->
    ...
    <!-- end 货品详情-->
    <!-- 新建 -->
    <el-dialog title="新建款式订货单" :visible.sync="newVisible" width="600px" @close="closeNewForm">
      <el-form :model="newStyleData" label-width="120px" :rules="newRules" ref="newForm">
        ...
        <el-form-item label="备注：" prop="Note">
          <el-input name="Note" type="textarea" @blur="newStyleData.Note = newStyleData.Note.trim()" v-model="newStyleData.Note" maxlength="200"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button name="btnSave" type="primary" :loading="createBtnLoading" @click="newSave('newForm')">保存</el-button>
        <el-button name="btnClose" @click="closeNewForm">取消</el-button>
      </div>
    </el-dialog>
    <!-- 作废 -->
    <obsolete v-if="obsoleteVisble" @visbleColse="visbleColse" @confirmObsolete="confirmObsolete" :data="operatingData" :visibleObs="obsoleteVisble"></obsolete>
    <!-- 审核 -->
    ...
    <el-dialog title="审核" :visible.sync="auditDialog" width="750px" @close="cancelAudit" v-if="auditDialog">
          ...
          <el-radio-group v-model="returnInfo.auditType" name="auditType">
            ...
            <el-row :gutter="10">
              ...
              <el-col :span="16" v-show="returnInfo.auditType === YNStatus.No">
                <el-input v-model="returnInfo.auditReson" placeholder="退回原因备注" @blur="returnInfo.auditReson = returnInfo.auditReson.trim()" :maxlength="200" name="auditReson"></el-input>
              </el-col>
            </el-row>
          </el-radio-group>
        </el-form-item>
        <div v-if="returnInfo.auditType == YNStatus.Yes && supplierList.length > 1 && operatingData[0].Preprice">
          <div class="colorRed">
          ...
          <el-form-item v-for="(item,index) in supplierList" :label="item.name + '：'" label-width="130" :key="index" style="margin-left: 30px;">
            <el-input style="width: 120px;" v-model="item.Preprice" @keyup.native="item.Preprice = $root.toFixed(item.Preprice, 2, true)" placeholder="应付款" name="auditReson" maxlength="10"></el-input>
          </el-form-item>
        </div>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="confirmAudit" :loading="$store.getters.is_loading" name="btnAuditStockloss">确 定</el-button>
        <el-button @click="cancelAudit" name="btnCancel">取 消</el-button>
      </span>
    </el-dialog>
    <!-- 取消审核 -->
    <cancel-Review :visibleCan="visibleCan" :data="operatingData" v-if="visibleCan" @confirmCancel="cancelConfirm" @visbleColse="cancelColse"></cancel-Review>
  </el-row>
</template>
  ...
  methods: {
    ...
    reviewStyleOrder(obj) {
      ...
      this.operatingData = obj
        ? [{ ...obj, orderNumber: obj.PurchaseCode }]
        : []
