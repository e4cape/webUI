  ...
  <el-row class="content">
    ...
    >
      ...
      <el-table-column
        ...
        show-overflow-tooltip
      >
        <template slot-scope="scope">{{scope.row.SendTime | filterDateMinutes}}</template>
      </el-table-column>
      <el-table-column
        prop="SendState"
        fixed="right"
      ...
      >
        <template slot-scope="scope">
          <span
            :class="scope.row.SendState | findKey(GoodsQualityOrderBasicStepState)"
          >{{GoodsQualityOrderBasicStepState.Types[scope.row.SendState] || '-'}}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" fixed="right" min-width="160">
        <template slot-scope="scope">
          <router-link
            name="btnCheck"
            :to="{path:'/purchase/censorshipProduct/censorshipCheck',query:{id: scope.row.QualityId}}"
            class="btn-link el-button el-button--text"
          >查看</router-link>
          <router-link
            name="btnEdit"
            :to="{path:'/purchase/censorshipProduct/censorship',query:{id: scope.row.QualityId}}"
            class="btn-link el-button el-button--text"
            v-if="scope.row.SendState == GoodsQualityOrderBasicStepState.Wait"
          >送检</router-link>
          <el-button
            name="btnComplete"
            type="text"
          ...
          <el-button
            ...
            v-if="scope.row.SendState == GoodsQualityOrderBasicStepState.Finish"
          >标记未完成</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import {
  STOCKING_API_GOODS_QUALITY_ORDER_BASIC_GETS2,
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.parameter.SendState = query.SendState || ''
    ...
    getData() {
      let parameter = Object.assign(this.queryForm, this.parameter, {
        SendTime1: this.parameter.SendTime[0],
        SendTime2: this.parameter.SendTime[1]
      })
      this.$store.commit('SET_TB_LOADING', true)
      STOCKING_API_GOODS_QUALITY_ORDER_BASIC_GETS2(parameter).then(res => {
