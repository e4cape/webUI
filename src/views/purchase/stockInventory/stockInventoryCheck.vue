<template>
  <el-row>
    <div class="panel" v-loading="$store.getters.tb_loading">
      <div class="panel-hd">
        <span class="title">查看进货入库单({{StuffType.Types[detail.StuffType]}})</span>
      </div>
      <div class="panel-bd">
              ...
              <tr>
                <td rowspan="5" class="state-img">
                  <img
                    ...
                    v-if="detail.State === StuffIntakeOrderBasicState.Draft"
                  >
                  <img
                    src="@/assets/images/auditing.png"
                    v-if="detail.State === StuffIntakeOrderBasicState.Wait"
                  >
                  <img
                    src="@/assets/images/audited.png"
                    v-if="detail.State === StuffIntakeOrderBasicState.Audit"
                  >
                  <img
                    src="@/assets/images/auditBack.png"
                    v-if="detail.State === StuffIntakeOrderBasicState.Reject"
                  >
                  <img
                    src="@/assets/images/abandon.png"
                    v-if="detail.State === StuffIntakeOrderBasicState.Abandon || detail.State === StuffIntakeOrderBasicState.Cancel"
                  >
                  <div>{{StuffIntakeOrderBasicState.Types[detail.State]}}</div>
                </td>
              </tr>
              <tr>
                <td class="tit">单号</td>
        ...
        <div class="p-x-10">
          ...
          >
            ...
            <el-table-column prop="StonePrice2" label="重单价" min-width="80" show-overflow-tooltip>
              <template slot-scope="scope">￥{{$root.toFloat(scope.row.StonePrice2)}}</template>
            </el-table-column>
            <el-table-column prop="Price" label="金额" min-width="80" show-overflow-tooltip>
              <template slot-scope="scope">￥{{$root.toFloat(scope.row.Price)}}</template>
            </el-table-column>
    ...
    <el-row style="margin-top: 10px; text-align: left; border: 0;">
      <el-button
        name="btnEdit"
        type="primary"
        @click="$router.push({path:'/purchase/stockInventory/stockInventoryEdit',query:{id: detail.IntakeId}})"
        v-if="detail.State == StuffIntakeOrderBasicState.Draft || detail.State == StuffIntakeOrderBasicState.Reject"
      >编辑</el-button>
      <el-button
        name="btnAudit"
        @click="showAudit"
...
import { StuffType, YNStatus, PurchaseType } from '@/enums/common'
import {
  ...
  STOCKING_API_STUFF_INTAKE_ORDER_ITEM_EXPORTGETSSTUFF
} from '@/apis/stocking'
...
export default {
  data() {
    return {
      ...
      cancelCheck: false,
      cancelName: 'cancel',
      stateCheck: StuffIntakeOrderBasicState.Audit, // 审核结果
      ExportVisible: false,
      ExportColumns: []
    }
  },
