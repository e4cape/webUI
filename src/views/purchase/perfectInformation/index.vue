  ...
  <el-row class="content">
    <el-form :model="queryForm" ref="search" lable-width="120px" class="item-lh-26" :inline="true">
      <search-panel @onSearch="onSearch" @onReset="onReset">
        <template slot="simpleSearch">
          <el-form-item prop="InfoState">
            <el-select
              name="InfoState"
              v-model="queryForm.InfoState"
              placeholder="所有状态"
              @change="onSearch"
              :filterable="true"
            >
        ...
        <template slot="seniorSearch">
          ...
          <el-form-item label="完成时间：" prop="InfoTime">
            <el-date-picker
              name="InfoTime"
              type="daterange"
              v-model="queryForm.InfoTime"
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span
            :class="scope.row.InfoState | findKey(GoodsQualityOrderBasicStepState)"
          >{{GoodsQualityOrderBasicStepState.Types[scope.row.InfoState] || '-'}}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" fixed="right" min-width="170">
        <template slot-scope="scope">
          <router-link
            name="btnCheck"
            :to="{path:'/purchase/perfectInformation/informationCheck',query:{id: scope.row.QualityId}}"
            class="btn-link el-button el-button--text"
          >查看</router-link>
          <router-link
            name="btnEdit"
            :to="{path:'/purchase/perfectInformation/informationEdit',query:{id: scope.row.QualityId}}"
            class="btn-link el-button el-button--text"
            v-if="scope.row.InfoState == GoodsQualityOrderBasicStepState.Wait"
          >完善资料</router-link>
          <el-button
            name="btnComplete"
            type="text"
          ...
          <el-button
            ...
            v-if="scope.row.InfoState == GoodsQualityOrderBasicStepState.Finish"
          >标记未完成</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import {
  STOCKING_API_GOODS_QUALITY_ORDER_BASIC_GETS4,
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.parameter.InfoState = query.InfoState || ''
      this.parameter.QualityType = query.QualityType || ''
      this.parameter.InfoTime = query.InfoTime || []
    ...
    getData() {
      let parameter = Object.assign(this.queryForm, this.parameter, {
        InfoTime1: this.parameter.InfoTime[0],
        InfoTime2: this.parameter.InfoTime[1]
      })
      this.$store.commit('SET_TB_LOADING', true)
      STOCKING_API_GOODS_QUALITY_ORDER_BASIC_GETS4(parameter).then(res => {
