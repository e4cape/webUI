<template>
  <el-row class="content">
    <el-form :model="queryForm" ref="search" lable-width="120px" class="item-lh-26" :inline="true">
      <search-panel @onSearch="onSearch" @onReset="onReset">
        <template slot="simpleSearch">
          <el-form-item prop="CertState">
            <el-select
              name="CertState"
              v-model="queryForm.CertState"
              placeholder="所有状态"
              @change="onSearch"
              :filterable="true"
            >
        ...
        <template slot="seniorSearch">
          ...
          <el-form-item label="完成时间：" prop="CertTime">
            <el-date-picker
              name="CertTime"
              type="daterange"
              v-model="queryForm.CertTime"
    ...
    >
      ...
      <el-table-column prop="SendQty" label="送检数量" min-width="80" show-overflow-tooltip></el-table-column>
      <el-table-column
        prop="CertTime"
        label="完成时间"
        min-width="120"
        ...
        show-overflow-tooltip
      >
        <template slot-scope="scope">{{scope.row.CertTime | filterDateMinutes}}</template>
      </el-table-column>
      <el-table-column
        prop="CertState"
        fixed="right"
      ...
      >
        <template slot-scope="scope">
          <span
            :class="scope.row.CertState | findKey(GoodsQualityOrderBasicStepState)"
          >{{GoodsQualityOrderBasicStepState.Types[scope.row.CertState] || '-'}}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" fixed="right" min-width="180">
        <template slot-scope="scope">
          <router-link
            name="btnCheck"
            :to="{path:'/purchase/certificateInput/certificateInputCheck',query:{id: scope.row.QualityId}}"
            class="btn-link el-button el-button--text"
          >查看</router-link>
          <router-link
            name="btnEntry"
            :to="{path:'/purchase/certificateInput/entryCertificate',query:{id: scope.row.QualityId}}"
            class="btn-link el-button el-button--text"
            v-if="scope.row.CertState == GoodsQualityOrderBasicStepState.Wait"
          >录入证书</router-link>
          <el-button
            name="btnComplete"
            type="text"
          ...
          <el-button
            ...
            v-if="scope.row.CertState == GoodsQualityOrderBasicStepState.Finish"
          >标记未完成</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import {
  STOCKING_API_GOODS_QUALITY_ORDER_BASIC_GETS3,
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.parameter.CertState = query.CertState || ''
      ...
      this.parameter.CertTime = query.CertTime || []
    ...
    getData() {
      let parameter = Object.assign(this.queryForm, this.parameter, {
        CertTime1: this.parameter.CertTime[0],
        CertTime2: this.parameter.CertTime[1]
      })
      this.$store.commit('SET_TB_LOADING', true)
      STOCKING_API_GOODS_QUALITY_ORDER_BASIC_GETS3(parameter).then(res => {
