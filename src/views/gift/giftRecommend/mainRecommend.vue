<template>
  <div>
    ...
    <el-alert title="" type="warning" :closable="false">
      <div slot>
        注：最多推荐20个礼品，推荐的礼品按顺序排在前面，如果商家没有上架，将自动隐藏，如果添加的礼品不足以区域显示，自动补充溢价空间高的代理礼品（零售价÷{{isOneNumberManyShopCompany || isOneNumberOneStore ? '采购价': '批发价'}}，值越大溢价空间越高。）
      </div>
    </el-alert>
    <!-- @module 选择礼品 -->
    ...
    getSettings () {
      GIFTING_API_PLATFORMSETTING_GETSETTINGS().then(res => {
        if (res.data.Code === 'CORRECT') {
          ...
          this.searchForm.settingOptionId = res.data.Data[0].settingOptionId
