<template>
  <div>
    <div class="p-10">
      <el-button name="btnSelectGift" type="primary" @click="showAddGift">选择礼品</el-button>
      ...
      <el-table-column prop="createTime" label="礼品名称" min-width="120" show-overflow-tooltip>
        <template slot-scope="scope">
          <div class="gift-info-cell">
            <img name="btnGetGiftDetail" :src="$root.settings.DOMAIN_IMAGE + scope.row.imageUrl" class="m-r-5 curter" @click="giftDetail(scope.row.giftId)">
            <p><el-button name="btnGetGiftDetail" type="text" @click="giftDetail(scope.row.giftId)">{{scope.row.giftName}}</el-button></p>
            <p><el-button name="btnGetGiftDetail" type="text" @click="giftDetail(scope.row.giftId)">{{scope.row.barCode}}</el-button></p>
          </div>
        </template>
      </el-table-column>
    ...
    <el-alert title="" type="warning" :closable="false">
      <div slot>
        注：最多推荐20个礼品，推荐的礼品按顺序排在前面，如果商家没有上架，将自动隐藏，如果添加的礼品不足以区域显示，自动补充溢价空间高的代理礼品（零售价÷批发价，值越大溢价空间越高。）
      </div>
    </el-alert>
    <!-- @module 选择礼品 -->
    <el-dialog title="选择礼品" :visible.sync="showSelectGift" width="1120px">
      <el-form :model="searchForm" ref="searchForm" label-width="70px" :inline="true">
        ...
        <el-form-item>
          <el-button name="btnSerch" type="primary" @click="search">搜索</el-button>
        </el-form-item>
      </el-form>
      <!-- @module 数据表格 -->
      <el-table :data="giftData" v-loading="loading" @sort-change="sortChange" element-loading-text="拼命加载中" @selection-change="handleGiftSelection">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button name="btnSelectGiftItem" type="primary" @click="selectGift(1)" :loading="$store.getters.is_loading">添加</el-button>
        <el-button name="btnAddCloseGiftItem" @click="selectGift(2)">添加并关闭</el-button>
...
import {
  ...
  GIFTING_API_PLATFORMSETTING_GETSETTINGS,
  GIFTING_API_PLATFORMSETTING_ADDGIFT,
...
export default {
  ...
  methods: {
    getSettings () {
      GIFTING_API_PLATFORMSETTING_GETSETTINGS().then(res => {
        if (res.data.Code === 'CORRECT') {
          this.settings = res.data.Data
          this.searchForm.settingOptionId = res.data.Data[1].settingOptionId
    ...
    selectGift (val) {
      // 推荐礼品
      if (this.data.length + this.selectedGiftId.length > 20) {
        return this.$message.error('最多推荐20个礼品')
      }
      if (this.recommendArray.toString() === this.selectedGiftId.toString() && this.selectedGiftId.length !== 0) {
