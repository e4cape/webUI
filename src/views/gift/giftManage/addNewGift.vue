<template>
  <div>
    ...
    <div class="dynamic">
      <div>
        <el-input v-model="specificationName" style="width:100px" maxlength="20" placeholder="规格"></el-input>
        <span>:</span>
      </div>
      <div>
        <div v-for="(item,index) in specificationList" :key="index">
          <el-input name="val1" v-model="item.val" style="width:120px" maxlength="20"></el-input>
          <el-button name="btnDelete1" type="danger" icon="el-icon-delete" circle @click="deleteDynamicItem(index,1)" v-if="specificationList.length > 1"></el-button>
        </div>
        <el-button type="primary" icon="el-icon-plus" circle @click="addDynamicItem(1)"></el-button>
      </div>
    </div>
    <div class="dynamic">
      <div>
        <el-input name="styleName" v-model="styleName" style="width:100px" maxlength="20" placeholder="样式"></el-input>
        <span>:</span>
      </div>
      <div>
        <div v-for="(item,index) in styleList" :key="index">
          <el-input name="val2"  v-model="item.val" style="width:120px" maxlength="20"></el-input>
          <el-button name="btnDelete2"  type="danger" icon="el-icon-delete" circle @click="deleteDynamicItem(index,2)" v-if="styleList.length > 1"></el-button>
        </div>
        <el-button name="btnAdd" type="primary" icon="el-icon-plus" circle @click="addDynamicItem(2)"></el-button>
      </div>
    </div>
    <div class="title">
      <h3>规格参数</h3>
    ...
    <el-form ref="parameterForm" :model="parameterObj">
      <el-table :data="parameterObj.list" empty-text="无规格参数" style="width: 100%">
        ...
        <el-table-column prop="contral" label="操作" width="80">
          <template slot-scope="scope">
            <span name="btnRemoveParameter" class="el-button el-button--text el-button--mini" @click="deleteParameterList(scope.$index)">删除</span>
          </template>
        </el-table-column>
      </el-table>
    </el-form>
...
import {
  ...
  GIFTING_API_GIFT_CREATEBYSTORE
} from '@/apis/gifting'
import {
  ...
  methods: {
    init() {
      if (!this.$route.query.categoryId) {
        ...
        this.form.categoryId = [
          Number(this.form.categoryId1),
          Number(this.form.categoryId2)
        ]
      } else {
    ...
    submit(submitType) {
      this.$refs['form'].validate(valid => {
        ...
        GIFTING_API_GIFT_CREATEBYSTORE(params).then(res => {
          if (res.data.Code === 'CORRECT') {
