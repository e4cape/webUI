<template>
  <div>
    ...
    <el-row :gutter="20" class="total-panel m-b-10" v-loading="$store.getters.tb_loading">
      ...
      <el-col :span="6">
        <div class="total price">
          <div class="number">￥{{Math.abs(numInfo.price).toFixed(2)}}</div>
    ...
    <el-table :data="data" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中" @sort-change="sortChange">
      <el-table-column prop="createTime" fixed label="时间" sortable min-width="120" show-overflow-tooltip></el-table-column>
      <el-table-column prop="orderId" label="充值单号" min-width="100" show-overflow-tooltip></el-table-column>
      <el-table-column prop="payNo" label="流水号" min-width="100" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column prop="price" label="充值金额" min-width="100" show-overflow-tooltip></el-table-column>
      <el-table-column prop="givePrice" label="赠送金额" min-width="100" show-overflow-tooltip></el-table-column>
      <el-table-column prop="createUser" label="操作人" show-overflow-tooltip></el-table-column>
...
import {
  ...
  GIFTING_API_ORDERRECHARGE_SEARCH
} from '@/apis/gifting'
import {
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      GIFTING_API_ORDERRECHARGE_SEARCH(this.queryForm).then(
        res => {
    ...
    getAllStore () {
      MERCHANT_API_DROPDOWN_STORELIST().then(res => {
        if (res.data.Code === 'CORRECT') {
          this.allStores = res.data.Data.Rows
        }
      })
    },
    initRoute() {
