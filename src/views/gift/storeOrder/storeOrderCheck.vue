    ...
    <div v-loading="$store.getters.tb_loading">
      ...
      <el-table :data="detail.orderItems" class="no-border">
          ...
          <template slot-scope="scope">
            <div class="style-item">
              <el-popover placement="right" v-if="scope.row.imageUrl" trigger="hover">
                ...
                <img name="btnGetDetailImg" slot="reference" class="small-img" @click="giftDetail(scope.row)" :src="$root.settings.DOMAIN_IMAGE + scope.row.imageUrl">
              </el-popover>
              <div class="text-item">
                <el-button type="text" name="btnGetDetail" @click="giftDetail(scope.row)">{{scope.row.giftName}}</el-button>
                <p>
        ...
        <el-table-column prop="retailPrice" label="单价" min-width="120" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>建议零售价：{{scope.row.retailPrice}}</span>
          </template>
        </el-table-column>
      ...
      <el-row style="margin: 20px; margin-left: 33px; text-align: left; border: 0;">
        <template v-if="detail.orderType == giftTypes.Self">
          <el-button name="btnAudit" v-if="detail.orderStatus == orderStatus.PendingAudit" type="primary" @click="audit(detail.orderId)">审核</el-button>
          <el-button name="btnSendThing" v-if="detail.orderStatus == orderStatus.PendingDelivery" type="primary" @click="showDelivery(detail.orderId)">发货</el-button>
          <el-button name="btnSendThingAgain" v-if="detail.orderStatus == orderStatus.PendingReceive" @click="showDelivery(detail.orderId)">重新发货</el-button>
          <el-button name="btnNullify" v-if="detail.orderStatus < orderStatus.PendingReceive" @click="invalid(detail.orderId)">作废</el-button>
        </template>
        <template v-else>
          <el-button name="btnAudit2" v-if="detail.orderStatus == orderStatus.PendingAudit" type="primary" @click="audit(detail.orderId)">审核</el-button>
          <el-button name="btnNullify2" v-if="detail.orderStatus == orderStatus.PendingAudit" @click="invalid(detail.orderId)">作废</el-button>
        </template>
      ...
      <el-dialog title="发货" :visible.sync="showDeliveryDialog" width="575px">
        ...
        <el-form :model="deliveryParams" ref="deliveryForm" label-position="right" label-width="100px" :rules="rules">
          ...
          <el-form-item label="收货方式：" prop="receiveType">
            <el-radio-group v-model="deliveryParams.receiveType">
          ...
          <el-form-item label="快递公司：" prop="expressType" v-if="deliveryParams.receiveType == 0">
            <el-select filterable v-model="deliveryParams.expressType" placeholder="请选择">
          ...
          <el-form-item label="快递单号：" prop="expressCode" v-if="deliveryParams.receiveType == 0">
            <el-input v-model="deliveryParams.expressCode" placeholder="20个英文字符以内" :maxlength="20"></el-input>
          </el-form-item>
          <el-form-item label="发货备注：">
            <el-input type="textarea" v-model="deliveryParams.expressNote" :maxlength="50" placeholder="50字以内"></el-input>
          </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
          <el-button name="btnSave" type="primary" @click="delivery">保 存</el-button>
          <el-button name="btnCancel" @click="showDeliveryDialog = false">关 闭</el-button>
        </span>
      </el-dialog>
      <!-- @module 发货 -->
      <!-- @module Dialog·打印订单 -->
...
export default {
  data() {
    ...
    return {
      ...
      detailShow: false,
      dialogLoading: false, // 礼品详情loading
      detailInfo: {
      },
      activeTab: '1',
      printDialog: false,
