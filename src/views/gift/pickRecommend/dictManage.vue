...
<script>
import {
  GIFTING_API_PLATFORMRECOMMEND_SAVECUSTOMCOLUMN,
  GIFTING_API_PLATFORMRECOMMEND_DELETECUSTOMCOLUMN,
  GIFTING_API_PLATFORMRECOMMEND_SETCUSTOMCOLUMNSORT
} from '@/apis/gifting'
...
export default {
  ...
  methods: {
    ...
    updateDict(row) {
      GIFTING_API_PLATFORMRECOMMEND_SAVECUSTOMCOLUMN(row)
        .then(res => {
          if (res.data.Code === 'CORRECT') {
            this.$message({
              message: '修改成功',
