<template>
  <el-row>
      ...
      <search-panel @onSearch="onSearch" @onReset="onReset">
        ...
        <template slot="simpleSearch">
          <el-form-item prop="status">
            <el-select name="orderStatus" filterable v-model="queryForm.orderStatus" @change="onSimpleSearch">
              ...
              <el-option v-for="item in orderStatus.Types" :key="item.key" :label="item.title" :value="item.key"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
            <el-input name="giftName"  v-model="queryForm.giftName" placeholder="礼品名称" @keyup.enter.native="onSearch">
              <el-button name="btnSearch"  slot="append" icon="el-icon-search"></el-button>
            </el-input>
          </el-form-item>
        </template>
        <template slot="seniorSearch">
          <el-row class="search-box no-border" type="flex">
            <el-col>
              <el-form-item label="来源：" prop="giftOrderType">
                <el-select name="giftOrderType" filterable v-model="queryForm.giftOrderType" placeholder="所有来源">
                  <el-option label="所有来源" value=""></el-option>
                  <el-option v-for="item in giftTypes.Types" :key="item.key" :label="item.title + '礼品'" :value="item.key"></el-option>
                </el-select>
              </el-form-item>
              ...
              <el-form-item label="下单时间：" prop="createTime">
                <el-date-picker name="createTime" v-model="queryForm.createTime" type="daterange" :clearable="false" :unlink-panels="true" placeholder="选择日期范围" :picker-options="$root.datePickerOptions"></el-date-picker>
              </el-form-item>
              ...
              <el-form-item label="发货时间：" prop="expressTime">
                <el-date-picker  name="expressTime"  v-model="queryForm.expressTime" type="daterange" :clearable="false" :unlink-panels="true" placeholder="选择日期范围" :picker-options="$root.datePickerOptions"></el-date-picker>
              </el-form-item>
              ...
              <el-form-item v-if="StoreCharacterType != EnumCharacterType.Store" label="供应商：" prop="supplierId">
                <el-select name="supplierId"  filterable v-model="queryForm.supplierId" placeholder="全部">
    ...
    <el-table :data="data" @sort-change="sortChange" :loading="$store.getters.tb_loading">
      ...
      <el-table-column prop="orderTypeText" label="礼品来源" min-width="120" show-overflow-tooltip></el-table-column>
    ...
    <export-filed-setter title="导出" :visible.sync="exportVisible" :api="filedsApi" apiKey="GIFTSALEORDER_GETEXCHANGEDETAILS" @submit="exportData"></export-filed-setter>
  </el-row>
</template>
<script>
...
import {
  ...
  GIFTING_API_GIFTSALEORDER_GETEXCHANGEDETAILS,
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      GIFTING_API_GIFTSALEORDER_GETEXCHANGEDETAILS(parameter).then(res => {
    ...
    exportData(fields) {
      ...
      GIFTING_API_STOREEXPORTER_EXPORTTOFILE({
        apiKey: 'GIFTSALEORDER_GETEXCHANGEDETAILS',
        exportFields: fields,
        exportType: 1,
