  ...
  <el-row>
    <el-row v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中">
      <div class="m-10 top-line-search">
        ...
        <span class="date-tips">最多选择2年</span>
      </div>
...
export default {
  ...
  methods: {
    getData() {
      ...
      STOCKING_API_REPORT_SETTLE_CHARTBYBUDGET({
        SettleBudgetBillType: SettleBudgetBillType.Quarterly,
    ...
    onSearch() {
      ...
      if (end - start > 1) {
        this.$message.warning('最多选择两年')
        flag = true
      }
      if (flag) {
        let year = new Date().getFullYear()
        this.startTime = new Date(Date.parse(year - 1 + '/01/01')) // 季报起始时间
        this.endTime = new Date(year + '/01/01') // 季报结束时间
      }
    ...
    initOptions(data) {
      ...
      data.forEach(res => {
        let date = res.Year + '年' + res.Date + '季度'
  ...
  beforeMount() {
    ...
    if (month <= 5) {
      ...
      this.startTime = new Date(year + '/' + (month - 5) + '/01') // 月报起始时间
    }
