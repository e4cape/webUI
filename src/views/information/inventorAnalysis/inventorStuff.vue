      ...
      <el-col :span="8">
        ...
        <template>
          <el-table :data="materialData">
            ...
            <el-table-column show-overflow-tooltip prop="LabelPrice" label="库存">
            ...
            <el-table-column show-overflow-tooltip label="占比">
              <template slot-scope="scope">
                <span style="margin-left: 5px">{{ scope.row.PerLabelPrice | absolutely}}</span>
              </template>
            </el-table-column>
          </el-table>
        </template>
      ...
      <el-col :span="8">
        ...
        <template>
          <el-table :data="categoryData">
            ...
            <el-table-column show-overflow-tooltip prop="PerLabelPrice" label="占比">
              <template slot-scope="scope">
                <span style="margin-left: 5px">{{ scope.row.PerLabelPrice | absolutely}}</span>
              </template>
            </el-table-column>
      ...
      <el-col :span="8">
        ...
        <template>
          <el-table :data="goldData">
            ...
            <el-table-column show-overflow-tooltip prop="PerLabelPrice" label="占比" min-width="">
              <template slot-scope="scope">
                <span style="margin-left: 5px">{{ scope.row.PerLabelPrice | absolutely}}</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
      ...
      STOCKING_API_REPORT_GOODS_STOCK_ANALYSISBYGOODSQTY(parameter).then(res => {
        if (res.data.Code === 'CORRECT') {
          ...
          this.materialData.forEach(item => {
            ...
            if (item.LabelPrice > 0) {
              data.push({
                value: this.$root.toFloat(item.LabelPrice),
    ...
    initPiedata(result, data) {
      ...
      pieData.title.subtext = '￥' + this.$root.toFloat(result.LabelPrice)
      return pieData
    },
    queryChange() {
      let parameter = {
