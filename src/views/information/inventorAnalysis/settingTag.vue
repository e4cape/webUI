<template>
  <el-dialog title="范围设置" :visible.sync="visible" width="750px">
    <div
      class="setting-tag"
      v-loading="loadingIf"
      ref="settingTag"
      ...
      <el-table :data="settingTagTableData">
        <el-table-column label="范围名称">
          <template slot-scope="scope">
            <el-input
              name="name"
              v-if="scope.row.editIf || currentSettingTagId == scope.row.settingTagId"
        ...
        <el-table-column label="范围">
          <template slot-scope="scope">
            <div v-if="scope.row.editIf || currentSettingTagId == scope.row.settingTagId">
              <el-input
                name="minValue"
                @keyup.native="scope.row.minValue = scope.row.minValue.replace(/[^\-?\d]/g,'')"
              ...
              <el-input
                name="maxValue"
                @keyup.native="scope.row.maxValue = scope.row.maxValue.replace(/[^\-?\d]/g,'')"
          ...
          <template slot-scope="scope">
            ...
            <div v-else>
              ...
              <el-button
                name="btnDel"
                type="text"
                @click="del(scope.row.settingTagId)"
                size="small"
                :disabled="!newIf"
              >删除</el-button>
            </div>
...
export default {
  props: ['visibleSettingTagModal', 'tagType'],
  data() {
    return {
      visible: this.visibleSettingTagModal,
  ...
  watch: {
    visible() {
      this.$emit('listenVisibleSettingTagModal')
    },
  },
  mounted() {
    this.$nextTick(() => {
      this.getCustomSettingTagsByTagType(this.tagType)
      // this.$refs.settingTag.getCustomSettingTagsByTagType(this.tagType);
    })
  },
  components: {
    // settingTag
