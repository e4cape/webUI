      ...
      <el-col :span="8">
        ...
        <template>
          <el-table :data="materialData">
            ...
            <el-table-column show-overflow-tooltip prop="FinanceQty" label="库存"></el-table-column>
      ...
      <el-col :span="8">
        ...
        <template>
          <el-table :data="goldData">
            ...
            <el-table-column show-overflow-tooltip prop="FinanceQty" label="库存"></el-table-column>
            <el-table-column show-overflow-tooltip prop="PerFinanceQty" label="占比" min-width="">
              <template slot-scope="scope">
                <span style="margin-left: 5px">{{ scope.row.PerFinanceQty | absolutely}}</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
      ...
      STOCKING_API_REPORT_GOODS_STOCK_ANALYSISBYGOODSQTY(parameter).then(res => {
        if (res.data.Code === 'CORRECT') {
          ...
          this.materialData.forEach(item => {
            ...
            if (item.FinanceQty > 0) {
              data.push({
                value: item.FinanceQty,
    ...
    initPiedata(result, data) {
      ...
      pieData.title.subtext = result.FinanceQty
      return pieData
    },
    queryChange() {
      let parameter = {
