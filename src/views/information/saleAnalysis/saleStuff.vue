<template>
  <el-row class="p-10">
    <div class="m-10 top-line-search">
      ...
      <el-select name="saleType" v-model="saleType" placeholder="应付金额" @change="queryChange">
        <el-option label="应付金额" :value="1"></el-option>
        <el-option label="实付金额" :value="3"></el-option>
      </el-select>
        ...
        <template>
          <el-table :data="materialData">
            ...
            <el-table-column show-overflow-tooltip label="金额">
              <template slot-scope="scope">
                ￥{{saleType === 1 ? $root.toFloat(scope.row.Price) : $root.toFloat(scope.row.CashPrice)}}
              </template>
            </el-table-column>
            <el-table-column show-overflow-tooltip label="占比">
              <template slot-scope="scope">
                <span style="margin-left: 5px">{{(saleType === 1 ? scope.row.PerPrice : scope.row.PerCashPrice) | absolutely}}</span>
              </template>
            </el-table-column>
          </el-table>
        </template>
...
export default {
  ...
  methods: {
      ...
      STOCKING_API_REPORT_SALE_ANALYSISBYSALEBOARD(parameter).then(res => {
        if (res.data.Code === 'CORRECT') {
          ...
          this.materialData.forEach(res => {
            if (res.Price > 0 && this.saleType === 1) {
              data.push({
                value: this.$root.toFloat(res.Price),
                name: res.EnumTypeName
              })
            }
            if (res.CashPrice > 0 && this.saleType === 3) {
              data.push({
                value: this.$root.toFloat(res.CashPrice),
                name: res.MaterialTypeName
              })
            }
          })
    ...
    initPiedata(result, data) {
      ...
      if ((this.saleType === 1 && result.Price !== 0) || (this.saleType === 3 && result.CashPrice !== 0)) {
        pieData.title.text = '总金额'
      } else {
        pieData.title.text = '暂无数据'
      }
      pieData.title.subtext = '￥' + (this.saleType === 1 ? this.$root.toFloat(result.Price) : this.$root.toFloat(result.CashPrice))
      return pieData
    },
    queryChange() {
      let parameter = {
