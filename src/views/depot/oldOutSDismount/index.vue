<template>
  <div class="content">
    ...
    <el-table 
      ...
      <el-table-column prop="WarehouseName" label="仓库" min-width="120" show-overflow-tooltip></el-table-column>
      <el-table-column prop="PartnerName" label="供应商" min-width="120" show-overflow-tooltip></el-table-column>
      <el-table-column prop="ReasonTypeDv" label="拆卸原因" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column prop="Quantity" label="旧货数量" min-width="100" show-overflow-tooltip></el-table-column>
      <el-table-column prop="GoldWeight" label="旧货金重" min-width="100" show-overflow-tooltip>
        <template slot-scope="scope">
          {{$root.toFloat(scope.row.GoldWeight, 3)}}g
        </template>
      </el-table-column>
      <!-- <el-table-column prop="Quantity" label="结算单" min-width="100" show-overflow-tooltip>
        <template slot-scope="scope">
          <router-link :to="`/depot/outSDismountBalance/index?id=${scope.row.Quantity}`">
            {{scope.row.Quantity}}
          </router-link>
        </template>
      </el-table-column> -->
      ...
      <el-table-column label="操作" min-width="250" fixed="right">
        <template slot-scope="scope">
          <router-link :to="{path:'/depot/oldOutSDismount/check',query:{id:scope.row.SplitId}}" class="btn-link el-button el-button--text">查看</router-link>
          <template v-if="scope.row.State == weiwGjunkSplitBasicState.Draft||scope.row.State == weiwGjunkSplitBasicState.Reject">
            <router-link :to="{path:'/depot/oldOutSDismount/edit',query:{id:scope.row.SplitId}}" class="btn-link el-button el-button--text">编辑</router-link>
    ...
    <dictManage v-if="dictDialog" :visible.sync="dictDialog" @listenDictSave="listenDictSave" :dialogTitle="dialogTitle" :dictType="settingDictionaryDictType.WeiwJunkSplitBasicReasonType" :dicts="reasonData"></dictManage>
...
import {
  STOCKING_API_WEIW_GJUNK_SPLIT_BASIC_GETS2,
  STOCKING_API_WEIW_GJUNK_SPLIT_BASIC_CREATE2 
  ...
  data() {
    return {
      ...
      queryForm: {
        // 查询表单数据
        SplitCode: '',
        SplitType: WeiwGjunkSplitBasicSplitType.Junk,
        WarehouseId: '0',
        ShelfId: '0',
        PartnerId: '0',
        PartnerName: '',
  ...
  methods: {
    init() {
      let query = JSON.stringify(this.$route.query) !== '{}' ? this.$route.query : {
        SplitCode: '',
        SplitType: this.WeiwGjunkSplitBasicSplitType.Junk,
        WarehouseId: '0',
        ShelfId: '0',
        PartnerId: '0',
        PartnerName: '',
    ...
    getData() {
      ...
      STOCKING_API_WEIW_GJUNK_SPLIT_BASIC_GETS2(this.queryForm).then(res => {
    ...
    createPurchase() {
      this.$refs['createForm'].validate(valid => {
        if (valid) {
          ...
          STOCKING_API_WEIW_GJUNK_SPLIT_BASIC_CREATE2 (
            Object.assign({}, this.createForm, {SplitType: this.WeiwGjunkSplitBasicSplitType.Junk})
          ).then(res => {
            if(res.data.Code == 'CORRECT'){
              this.createDialog = false
              this.$message.success('提交成功！')
              this.$router.push({
                path: '/depot/oldOutSDismount/edit',
    ...
    getReasons(){
      // this.$store.dispatch('GET_REASONS_DROPLIST', {DictType: this.settingDictionaryDictType.WeiwJunkSplitBasicReasonType, State: 0})
      MERCHANT_API_DROPDOWN_SETTINGDICTIONARYLIST({
