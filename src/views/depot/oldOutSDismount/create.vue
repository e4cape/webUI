<template>
  <div>
    <el-dialog title="选择旧货" :visible.sync="visible" @close="$emit('listenCreateDialog', success)">
      ...
      <div class="panel-bd">
        <!-- 表格 -->
        <el-table :data="junkList" @row-click="selectionChange" @sort-change="sortChange" highlight-current-row v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中">
          ...
          <el-table-column prop="RecallGoldPrice" label="回收金价(元/g)" min-width="100" show-overflow-tooltip>
            <template slot-scope="scope">
              ￥{{$root.toFloat(scope.row.RecallGoldPrice)}}g
            </template>
          </el-table-column>
...
export default {
  // props: ['createDialog', 'SplitId', 'listenCreateDialog', 'data'],
  props: {
    createDialog: {
      default: false,
      type: Boolean
  ...
  methods: {
    ...
    getJunkList() {
      this.queryForm.WarehouseId = this.data.WarehouseId
      this.queryForm.ShelfId = this.data.ShelfId
    ...
    selectAddJunk(flag) {
      ...
      if (this.selections.length === 0) {
        ...
        // this.visible = false
        // this.$emit('listenCreateDialog', 'createDialog', this.selections)
        ...
        STOCKING_API_WEIW_GJUNK_SPLIT_ITEM_CREATE({
          ...
          this.$store.commit('SET_BTN_LOADING', this.success)
        })
      }
    },
  ...
  watch: {
    'queryForm.IsGold': 'search',
    'queryForm.MaterialType': 'search',
    'queryForm.CategoryType': 'search',
    'queryForm.GoldType': 'search',
  },
  components: {
    pagination
  }
