  ...
  <div class="content">
    <el-form :model="queryForm" ref="search" @keyup.enter.native="onSearch" lable-width="120px" class="item-lh-26" :inline="true">
      <search-panel @onSearch="onSearch" @onReset="onReset">
        <template slot="btnBox">
          <el-button type="primary" @click="createDialog = true, createForm.ActualDate = $options.filters.filterAllDate(new Date())" name="btnCreate">新建</el-button>
          <el-dropdown @command="multiAudit" v-if="data.length">
        ...
        <template slot="simpleSearch">
          <el-form-item prop="State">
            <el-select v-model="queryForm.State" @change="onSearch" name="State">
              ...
              <template v-for="(item,index) in HalfAllotOrderOutakeState.Types">
                <el-option v-if="Number(index) !== HalfAllotOrderOutakeState.Cancel" :key="index" :label="item" :value="String(index)"></el-option>
              </template>
            </el-select>
          </el-form-item>
        ...
        <template slot="seniorSearch">
          ...
          <el-form-item prop="IntakeState" label="收货情况：">
            <el-select v-model="queryForm.IntakeState" :filterable="true" name="IntakeState">
              ...
              <el-option v-for="(item,index) in HalfAllotOrderIntakeState.Types" :key="index" :label="item" :value="String(index)"></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item prop="ActualDate" label="业务日期：" name="checkTime">
    ...
    <el-table :data="data" @sort-change="sortChange" @selection-change="selectionChange" ref="tableData" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中">
      ...
      <el-table-column prop="State" label="状态" min-width="100" fixed="right">
        <template slot-scope="scope">
          <el-tooltip :disabled="!scope.row.CheckNote" class="item" :content="scope.row.CheckNote" placement="top" effect="light">
            <span :class="scope.row.State | findKey(HalfAllotOrderOutakeState)">{{HalfAllotOrderOutakeState.Types[scope.row.State]}}</span>
          </el-tooltip>
        </template>
      </el-table-column>
      <el-table-column prop="IntakeState" label="收货情况" min-width="160" show-overflow-tooltip fixed="right">
        <template slot-scope="scope">
          <el-tooltip
            :disabled="!(scope.row.CheckNote && scope.row.IntakeState === HalfAllotOrderIntakeState.Reject || scope.row.IntakeTime && scope.row.IntakeState === HalfAllotOrderIntakeState.Audit)"
            class="item"
            :content="scope.row.IntakeState === HalfAllotOrderIntakeState.Reject ? scope.row.CheckNote : $options.filters.filterDateMinutes(scope.row.IntakeTime)"
            placement="top"
            effect="light"
          >
            <span>{{HalfAllotOrderIntakeState.Types[scope.row.IntakeState]||'--'}}</span>
          </el-tooltip>
        </template>
      </el-table-column>
      <el-table-column label="操作" min-width="160" fixed="right">
        <template slot-scope="scope">
          <router-link :to="{path:'/depot/semiappropout/check',query:{id:scope.row.OutakeId}}" class="btn-link el-button el-button--text" name="btnShowDetail">查看</router-link>
          <template v-if="scope.row.State === HalfAllotOrderOutakeState.Reject || scope.row.State === HalfAllotOrderOutakeState.Draft">
            <router-link :to="{path:'/depot/semiappropout/edit',query:{id:scope.row.OutakeId}}" class="btn-link el-button el-button--text" name="btnAppropOutEdit">编辑</router-link>
            <el-button type="text" @click="appropOutAbandon(scope.row)" name="btnAppropOutAbandon">作废</el-button>
          </template>
          <el-button type="text" @click="appropOutAudit(scope.row)" v-if="scope.row.State === HalfAllotOrderOutakeState.Wait" name="btnAppropOutAudit">审核</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- End 数据表格 -->
    ...
    <!-- @module 新建半成品调拨出库单 -->
    <el-dialog title="新建半成品调拨出库单" :visible.sync="createDialog" @close="resetForm">
...
<script>
import { YNStatus, ShippingType } from '@/enums/common.js'
import {
...
import {
  STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_GETS,
  STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_CREATE
...
export default {
  ...
  methods: {
    formatter(row, column, val) { // 表格格式化
    ...
    getData() {
      ...
      STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_GETS(this.parameters).then(res => {
    ...
    createappropOut() {
      this.$refs['createForm'].validate(valid => {
        if (valid) {
          ...
          STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_CREATE(this.createForm).then(
            res => {
              if (res.data.Code === 'CORRECT') {
                this.$message({
                  type: 'success',
    ...
    multiAudit() {
      ...
      if (this.selections.length === 0 ||
        this.selections.some(
          item => item.State !== HalfAllotOrderOutakeState.Wait
        )
      ) {
