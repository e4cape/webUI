...
import {
  STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_AUDITS,
  STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_REJECTS,
  STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_AUDIT,
  STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_REJECT
    ...
    auditAdjust() {
      ...
      if (this.data.length > 1) {
        ...
        if (this.auditType == YNStatus.Yes) {
          result = STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_AUDITS(param)
        } else {
          result = STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_REJECTS(param)
        }
      } else {
        let param = {
          CheckNote: this.auditReson,
        ...
        if (this.auditType == YNStatus.Yes) {
          result = STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_AUDIT(param)
        } else {
          result = STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_REJECT(param)
        }
      }
