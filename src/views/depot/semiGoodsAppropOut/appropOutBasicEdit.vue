<template>
  <div>
    ...
    <el-dialog title="修改半成品调拨出库单" :visible="visible" @update:visible="$emit('update:visible', $event)">
      <el-form label-position="right" label-width="100px" :model="editParam" :rules="editRules" ref="editParam" v-loading="loading">
        ...
        <el-form-item label="收货位置" required>
          <div class="position">
            ...
            <el-form-item class="item" >
              <el-select v-model="editForm.ShelfName2" disabled :placeholder="editForm.ShelfName2">
...
import { STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_UPDATE } from '@/apis/stocking.js'
    ...
    editappropOut() {
      this.$refs['editParam'].validate(valid => {
        if (valid) {
          ...
          STOCKING_API_HALF_ALLOT_ORDER_OUTAKE_UPDATE(this.editParam).then(
            res => {
              if (res.data.Code === 'CORRECT') {
                this.$message({
                  message: '修改成功',
