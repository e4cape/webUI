<template>
  <!-- @module Dialog·审核 -->
  <el-dialog title="审核" :visible.sync="visible" width="600px" @close="$emit('listenAuditDialog', 'auditDialog', success)">
...
import {
  STOCKING_API_WEIW_GJUNK_SETTLE_BASIC_AUDITS,
  STOCKING_API_WEIW_GJUNK_SETTLE_BASIC_AUDIT,
  STOCKING_API_WEIW_GJUNK_SETTLE_BASIC_REJECTS,
  STOCKING_API_WEIW_GJUNK_SETTLE_BASIC_REJECT
    ...
    auditAdjust() {
      ...
      if(this.data.length == 1){
        ...
        this.data.forEach(item => {
          SettleIds.push({SettleId: item.SettleId, CheckNote: this.auditReson})
        })
      ...
      if(this.isOne){
        if (this.auditType === this.YNStatus.No) {
          apiMethod = STOCKING_API_WEIW_GJUNK_SETTLE_BASIC_REJECT
        }else{
          apiMethod = STOCKING_API_WEIW_GJUNK_SETTLE_BASIC_AUDIT 
        }
      }else{
        if (this.auditType === this.YNStatus.No) {
          apiMethod = STOCKING_API_WEIW_GJUNK_SETTLE_BASIC_REJECTS
        }else{
          apiMethod = STOCKING_API_WEIW_GJUNK_SETTLE_BASIC_AUDITS
        }
      }
