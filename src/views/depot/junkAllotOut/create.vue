<template>
  <div>
    <el-dialog title="选择旧货" :visible.sync="visible" @close="$emit('listenCreateDialog', true)">
      <div class="cj_head">
        <el-row :gutter="2">
          <el-col :span="4">
      ...
      <div class="panel-bd">
        ...
        <el-table 
          ...
          <el-table-column prop="MaterialType" label="材质" min-width="60" show-overflow-tooltip>
            <template slot-scope="scope">
              {{$store.getters.materialType.Types[scope.row.MaterialType]}}
            </template>
          </el-table-column>
          <el-table-column prop="CategoryType" label="品类" min-width="60" show-overflow-tooltip>
            <template slot-scope="scope">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button @click="$emit('listenCreateDialog', true)" name="btnClose">关闭</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getJunkList() {
      ...
      this.queryForm.WarehouseId = this.detail.WarehouseId1
      this.queryForm.ShelfId = this.detail.ShelfId1
    ...
    selectAddJunk(flag) {
      ...
      STOCKING_API_JUNK_ALLOT_ORDER_ITEM_CREATE({
        ...
        if(res.data.Code == 'CORRECT'){
          if(flag){
            // this.visible = false
            this.$emit('listenCreateDialog', true)
          }
          // this.$emit('listenCreateDialog')
          this.$message({
            type: 'success',
            message: '添加成功！'
          })
