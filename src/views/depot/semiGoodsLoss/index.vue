  ...
  <div class="content">
    ...
    <el-table :data="data" @sort-change="sortChange" @selection-change="selectionChange" ref="tableData" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中">
      ...
      <el-table-column label="操作" min-width="160" fixed="right">
        <template slot-scope="scope">
          <router-link :to="{path:`/depot/semistockloss/check`,query:{id:scope.row.ReportId}}" class="btn-link el-button el-button--text" name="btnCheck">查看</router-link>
          <template
          ...
          >
            <router-link :to="{path:`/depot/semistockloss/edit`,query:{id:scope.row.ReportId, deskId:scope.row.DeskId}}" class="btn-link el-button el-button--text" name="btnEdit">编辑</router-link>
    ...
    <!-- @module 新建 -->
    <el-dialog title="新建半成品报损单" :visible.sync="createDialogVisible" @close="resetForm">
      <el-form :model="createForm" label-position="right" label-width="100px" :rules="rules" ref="createForm">
        <el-row>
          <el-col :span="24">
            <el-form-item label="报损位置" required>
              <div class="position">
...
import {
  STOCKING_API_HALF_COUNT_REPORT_BASIC_GETS,
  STOCKING_API_HALF_COUNT_REPORT_BASIC_CREATE
...
export default {
  data() {
    return {
      ...
      queryForm: {
        // 查询表单数据
        ReportType: HalfCountReportBasicReportType.Loss,
        SourceType: '0',
        WarehouseId: '0',
        ShelfId: '0',
        CreateUserId: '0',
  ...
  methods: {
    ...
    getData() {
      ...
      STOCKING_API_HALF_COUNT_REPORT_BASIC_GETS(this.parameters).then(res => {
    ...
    showCreateDialog() {
      ...
      this.createForm.ReportType = HalfCountReportBasicReportType.Loss
    },
    create() {
      this.$refs['createForm'].validate(valid => {
        if (valid) {
          this.$store.commit('SET_BTN_LOADING', true)
          STOCKING_API_HALF_COUNT_REPORT_BASIC_CREATE(this.createForm).then(
            res => {
              if (res.data.Code === 'CORRECT') {
                this.createDialogVisible = false
                this.$router.push({
                  path: '/depot/semistockloss/edit',
                  query: {
                    id: res.data.Data
                  }
                })
    ...
    multiAudit() {
      if (
        ...
        this.$message.warning('请选择待审核状态的半成品报损单')
      }
    },
    audit(row) {
      this.selections = [row]
