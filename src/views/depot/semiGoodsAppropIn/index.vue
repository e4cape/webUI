<template>
  <div class="content">
      ...
      <search-panel @onSearch="onSearch" @onReset="onReset">
        ...
        <template slot="simpleSearch">
          <el-form-item prop="State">
            <el-select v-model="queryForm.State" @change="onSearch" name="receiveType">
        ...
        <template slot="seniorSearch">
          ...
          <el-form-item label="单据编号：" prop="OutakeCode">
            <el-input v-model="queryForm.OutakeCode" :maxlength="50" name="appropCode"></el-input>
          </el-form-item>
    ...
    <el-table :data="data" @sort-change="sortChange" @selection-change="selectionChange" ref="tableData" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中">
      ...
      <el-table-column prop="ActualDate" label="业务日期" :formatter="formatter" min-width="100" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column prop="State" label="状态" min-width="90" fixed="right">
        <template slot-scope="scope">
          <el-tooltip :disabled="!scope.row.CheckNote" class="item" :content="scope.row.CheckNote" placement="top" effect="light">
            <span>{{HalfAllotOrderIntakeState.Types[scope.row.State]}}</span>
          </el-tooltip>
        </template>
      </el-table-column>
      <el-table-column label="操作" min-width="200" fixed="right">
        <template slot-scope="scope">
          <router-link :to="{path:'/depot/semiappropin/check',query:{id:scope.row.IntakeId}}" class="btn-link el-button el-button--text" name="btnShow">查看</router-link>
          <template v-if="scope.row.State === HalfAllotOrderIntakeState.Wait">
    ...
    <order-intake :visible.sync="orderIntakeVisible" :data="selectRow" @listenOrderIntakeVisible="getData"></order-intake>
...
import { STOCKING_API_HALF_ALLOT_ORDER_INTAKE_GETS } from '@/apis/stocking.js'
  ...
  methods: {
    formatter(row, column, val) {
      switch (column.property) {
        ...
        case 'ReceiveUser':
          return HalfAllotOrderIntakeState.Audit === row.State ? val : '-'
        case 'AllotWgt':
    ...
    getData() {
      ...
      STOCKING_API_HALF_ALLOT_ORDER_INTAKE_GETS(this.parameters).then(res => {
    ...
    multiOperation(command) {
      if (
        ...
        this.selections.every(
          item => item.State === HalfAllotOrderIntakeState.Wait
        )
      ) {
        if (command === 'Reject') {
        ...
        this.$message.error('请选择待收货状态的半成品调拨入库单')
      }
    },
