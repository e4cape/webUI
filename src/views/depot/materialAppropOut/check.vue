<template>
  <div class="content">
    <div class="panel m-b-0">
      <div class="panel-hd">
        <span class="title">查看调拨出库单({{StuffType.Types[detail.StuffType]}})</span>
      </div>
      <div class="panel-bd">
            ...
            <tbody>
              <tr>
                <td rowspan="6" class="tc">
                  <img src="@/assets/images/draft.png" v-if="detail.State === StuffAllotOrderOutakeState.Draft">
                  <img src="@/assets/images/auditing.png" v-if="detail.State === StuffAllotOrderOutakeState.Wait">
                  <img src="@/assets/images/audited.png" v-if="detail.State === StuffAllotOrderOutakeState.Audit">
                  <img src="@/assets/images/auditBack.png" v-if="detail.State === StuffAllotOrderOutakeState.Reject">
                  <img src="@/assets/images/abandon.png" v-if="detail.State === StuffAllotOrderOutakeState.Abandon">
                  <div>{{StuffAllotOrderOutakeState.Types[detail.State]}}</div>
                </td>
              </tr>
              <tr>
                <td class="tit">单号：</td>
        ...
        <div class="p-x-10">
          ...
          <el-table :data="goodsData" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中" class="m-b-10">
            ...
            <template v-else-if="detail.StuffType === StuffType.Stone">
    ...
    <div class="buttons">
      <template v-if="detail.State === StuffAllotOrderOutakeState.Reject || detail.State === StuffAllotOrderOutakeState.Draft">
        <router-link :to="{path:'/depot/materialappropout/edit',query:{id: detail.OutakeId}}" name="btnEdit">
          <el-button type="primary">编辑</el-button>
        </router-link>
        <el-button @click="abandonDialog = true" name="btnAbandon">作废</el-button>
      </template>
      <el-button type="primary" @click="auditDialog = true" v-if="detail.State === StuffAllotOrderOutakeState.Wait" name="btnAudit">审核</el-button>
...
import { StuffAllotOrderOutakeState } from '@/enums/stocking.js'
import {
