  ...
  <div class="content">
        ...
        <template slot="btnBox">
          ...
          <el-dropdown @command="multiAudit">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item :command="stuffType.Gold">金料报损</el-dropdown-item>
              <el-dropdown-item :command="stuffType.Stone">石料报损</el-dropdown-item>
              <el-dropdown-item :command="stuffType.Part">配件报损</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
          ...
          <el-form-item prop="CreateUserId" label="创建人：">
            <el-select v-model="queryForm.CreateUserId" :filterable="true" placeholder="全部" name="createUserId">
              ...
              <template v-for="(item, index) in $store.getters.users">
                <el-option v-if="item.UserState === securityUserStates.Audit && item.TrueName" :key="index" :label="item.TrueName" :value="String(item.UserId)"></el-option>
              </template>
            </el-select>
          </el-form-item>
    ...
    <el-table :data="dataTable" @sort-change="sortChange" @selection-change="selectionChange" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中">
      ...
      <el-table-column prop="WarehouseName" label="位置" min-width="100" show-overflow-tooltip>
        <template slot-scope="scope">
          {{scope.row.WarehouseName || '仓库'}}
        </template>
      </el-table-column>
      <el-table-column prop="WarehouseName" label="类别" min-width="100" show-overflow-tooltip>
        <template slot-scope="scope">
          {{stuffType.Types[scope.row.StuffType]}}
        </template>
      </el-table-column>
      <el-table-column prop="SourceType" label="来源" min-width="120" show-overflow-tooltip>
        <template slot-scope="scope">
          {{stuffCountReportBasicSourceType.Types[scope.row.SourceType]}}
        </template>
      </el-table-column>
      ...
      <el-table-column prop="State" label="状态" min-width="100" fixed="right">
        <template slot-scope="scope">
          <template v-if="(scope.row.State === stuffCountReportBasicState.Abandon || scope.row.State === stuffCountReportBasicState.Reject || scope.row.State === stuffCountReportBasicState.Cancel)&&scope.row.CheckNote">
      ...
      <el-table-column label="操作" min-width="160" fixed="right">
        <template slot-scope="scope">
          <router-link :to="{path:'/depot/stockloss/check',query:{id:scope.row.ReportId,StuffType: scope.row.StuffType}}" class="btn-link el-button el-button--text" name="btnCheck">查看</router-link>
          <template v-if="scope.row.State === stuffCountReportBasicState.Reject || scope.row.State === stuffCountReportBasicState.Draft && scope.row.SourceType !== stuffCountReportBasicSourceType.Taking">
            <router-link :to="{path:'/depot/stockloss/edit',query:{id: scope.row.ReportId, deskId:scope.row.DeskId, StuffType: scope.row.StuffType}}" class="btn-link el-button el-button--text" name="btnEdit">编辑</router-link>
            <el-button type="text" @click="openDialog(scope.row, 'abandonDialog')" name="btnAbandon">作废</el-button>
          </template>
          <el-button type="text" @click="openDialog(scope.row, 'auditDialog')" v-if="scope.row.State === stuffCountReportBasicState.Wait" name="btnAudit">审核</el-button>
          <el-button type="text" @click="openDialog(scope.row, 'cancelDialog')" v-if="scope.row.State === stuffCountReportBasicState.Audit" name="btnCancel">取消审核</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- End 数据表格 -->
    ...
    <el-dialog title="新建报损单" :visible.sync="createDialogVisible" @close="resetForm">
      ...
      <span slot="footer" class="dialog-footer tc">
        <el-button type="primary" @click="createLoss" :loading="$store.getters.is_loading" name="createLoss">确 定</el-button>
...
import {
  STOCKING_API_STUFF_COUNT_REPORT_BASIC_GETS,
  STOCKING_API_STUFF_COUNT_REPORT_BASIC_CREATE
...
export default {
  data() {
    return {
      ...
      queryForm: {
        // 查询表单数据
        ReportType: StuffCountReportBasicReportType.Loss,
        ReportCode: '',
        CountCode: '',
        WarehouseId: '0',
        ShelfId: '0',
  ...
  methods: {
    init() {
      let query = JSON.stringify(this.$route.query) !== '{}' ? this.$route.query : {
        ReportType: this.stuffCountReportBasicReportType.Loss,
        ReportCode: '',
        CountCode: '',
        WarehouseId: '0',
        ShelfId: '0',
    ...
    getData() {
      ...
      STOCKING_API_STUFF_COUNT_REPORT_BASIC_GETS(this.queryForm).then(res => {
    ...
    createLoss() {
      ...
      this.$refs['createForm'].validate(valid => {
        if (valid) {
          this.createForm.ReportType = this.stuffCountReportBasicReportType.Loss
          this.createForm.StuffType = this.currType
          this.$store.commit('SET_BTN_LOADING', true)
          STOCKING_API_STUFF_COUNT_REPORT_BASIC_CREATE(this.createForm).then(res => {
            if (res.data.Code === 'CORRECT') {
              this.createDialogVisible = false
              this.$router.push({
                path: '/depot/stockloss/edit',
                query: {
                  id: res.data.Data, StuffType: this.createForm.StuffType
                }
              })
            } else {
    ...
    stocklossAbandon(row) {
      this.abandonDialog = true
      this.abandonStockLoss = row
    },
    stocklossCancel(row) {
      this.cancelDialog = true
      this.cancelStockLoss = row
    },
    resetForm() {
      this.createForm = {
        ReportType: '',
  ...
  watch: {
    ...
    '$store.getters.wareHouses' () {
      this.houseOptions = JSON.parse(JSON.stringify(this.$store.getters.wareHouses).replace(/Id/g, 'value').replace(/Value/g, 'label').replace(/Childrens/g, 'children'))
    }
  },
  components: {
    searchPanel,
