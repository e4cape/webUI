<template>
  <div class="content">
    <div class="panel">
      <div class="panel-hd">
        <span class="title">编辑报溢单({{StuffType.Types[$route.query.StuffType]}})</span>
        <span class="title fr">
        ...
        <div class="p-x-10">
          ...
          <el-table 
            ...
            <el-table-column v-if="currTabs == StuffType.Stone" key="31" prop="StonePackageNo" min-width="100">
              ...
              <template v-if="scope.$index >= currentStartIndex && scope.$index <= currentEndIndex" slot-scope="scope">
                <el-input v-model="scope.row.StonePackageNo" :maxlength="30" @blur="scope.row.StonePackageNo = scope.row.StonePackageNo.trim()" @input="handleFocus(scope.row)"></el-input>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="CostPrice" label="金额" min-width="130">
              <template slot-scope="scope">
                <el-input v-model="scope.row.CostPrice" :maxlength='10' @keyup.native="scope.row.CostPrice = $root.toFixed(scope.row.CostPrice, 2)" @input="handleFocus(scope.row)"></el-input>
              </template>
            </el-table-column>
    ...
    <!-- @module 修改 -->
    <el-dialog title="修改报溢单" :visible.sync="editDialog" @close="resetForm">
      <el-form :model="editForm" label-position="right" label-width="100px"  ref="editForm">
        <el-row>
          <el-col :span="8">
            <el-form-item label="报溢位置：" prop="WarehouseId">
      ...
      <span slot="footer" class="dialog-footer tc">
        <el-button type="primary" @click="updateLoss" :loading="$store.getters.is_loading" name="updateLoss">确 定</el-button>
      ...
      <el-form :model="copyLineForm" @keyup.enter.native="copySure" @submit.native.prevent>
        ...
        <!-- <el-form-item label="位置：">
          <el-radio-group name="local" v-model="copyLineForm.local">
            ...
            <el-radio label="2">复制到第 <el-input-number name="rowsNum" v-model="copyLineForm.current" :min="0" :max="data.length" :controls="false" style="width: 50px"></el-input-number> 行</el-radio>
          </el-radio-group>
        </el-form-item> -->
      </el-form>
      <span slot="footer" class="dialog-footer">
...
import { 
  STOCKING_API_STUFF_COUNT_REPORT_BASIC_GET1,
  STOCKING_API_STUFF_COUNT_REPORT_ITEM_GETS,
  STOCKING_API_STUFF_COUNT_REPORT_BASIC_UPDATE1,
  STOCKING_API_STUFF_COUNT_REPORT_BASIC_WAIT1,
  STOCKING_API_STUFF_COUNT_REPORT_ITEM_CREATE1,
  STOCKING_API_STUFF_COUNT_REPORT_ITEM_UPDATE1,
  STOCKING_API_STUFF_COUNT_REPORT_ITEM_DELETE1
...
export default {
  ...
  methods: {
    ...
    init () {
      ...
      this.ReportId = parseInt(query.id)||0
      if (!this.ReportId) {
        this.dataError()
      } else {
        this.getDetail()
    ...
    updateLoss(){
      STOCKING_API_STUFF_COUNT_REPORT_BASIC_UPDATE1({ReportId: this.detail.ReportId, Note: this.editForm.Note}).then(res => {
        if(res.data.Code == 'CORRECT'){
      ...
      for (var i = 0; i < this.data.length; i++) {
        ...
        if(this.data[i].isEdit || this.data[i].isError){
          ...
          if (this.data[i].isEdit || this.data[i].isError) {
            ...
            console.log('this.data[i]1', this.data[i])
            if (this.data[i].ItemId) {
              res = await STOCKING_API_STUFF_COUNT_REPORT_ITEM_UPDATE1(obj)
            } else {
              res = await STOCKING_API_STUFF_COUNT_REPORT_ITEM_CREATE1(obj)
            }
