  ...
  <div class="content">
    ...
    <el-table :data="data" @sort-change="sortChange" @selection-change="selectionChange" ref="tableData" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中">
      ...
      <el-table-column label="操作" min-width="160" fixed="right">
        <template slot-scope="scope">
          <router-link :to="{path: '/depot/goodstockloss/check',query:{id:scope.row.ReportId}}" class="btn-link el-button el-button--text" name="btnCheck">查看</router-link>
          <template
          ...
          >
            <router-link :to="{path: '/depot/goodstockloss/edit',query:{id:scope.row.ReportId}}" class="btn-link el-button el-button--text" name="btnEdit">编辑</router-link>
...
import {
  STOCKING_API_GOODS_COUNT_REPORT_BASIC_GETS,
  STOCKING_API_GOODS_COUNT_REPORT_BASIC_CREATE
...
export default {
  data() {
    return {
      ...
      queryForm: {
        // 查询表单数据
        ReportType: GoodsCountReportBasicReportType.Loss,
        SourceType: '0',
        ObjectType: 0,
        WarehouseId: '0',
        ShelfId: '0',
  ...
  methods: {
    ...
    showCreateDialog() {
      ...
      this.createForm.ReportType = GoodsCountReportBasicReportType.Loss
    },
    create() {
      this.$refs['createForm'].validate(valid => {
        if (valid) {
          ...
          STOCKING_API_GOODS_COUNT_REPORT_BASIC_CREATE(this.createForm).then(
            res => {
              if (res.data.Code === 'CORRECT') {
                this.createDialogVisible = false
                this.$router.push({
                  path: '/depot/goodstockloss/edit',
                  query: {
                    id: res.data.Data
                  }
                })
    ...
    getData() {
      ...
      STOCKING_API_GOODS_COUNT_REPORT_BASIC_GETS(this.parameters).then(res => {
        if (res.data.Code === 'CORRECT') {
