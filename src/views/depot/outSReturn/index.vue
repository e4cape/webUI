<template>
  <div class="content">
      ...
      <search-panel @onSearch="onSearch" @onReset="onReset">
        ...
        <template slot="simpleSearch">
          <el-form-item prop="State">
            <el-select v-model="queryForm.State" placeholder="所有状态">
              ...
              <el-option v-for="(item,index) in weiwStuffReturnBasicState.Types" :key="index" :label="item" :value="index"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item prop="ReturnCode">
            <el-input v-model="queryForm.ReturnCode" :maxlength="50" placeholder="单据编号" @keyup.enter.native="onSearch">
        ...
        <template slot="seniorSearch">
          <el-form-item label="单据编号：" prop="ReturnCode">
            <el-input v-model="queryForm.ReturnCode" :maxlength="50" @keyup.enter.native="onSearch"></el-input>
          </el-form-item>
    ...
    <el-table 
      ...
      <el-table-column prop="ReturnCode" label="单据编号" width="140" sortable="custom" show-overflow-tooltip fixed="left"></el-table-column>
      ...
      <el-table-column prop="ReasonTypeDv" label="退料原因" min-width="80" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column prop="State" label="状态" width="80" fixed="right">
        <template slot-scope="scope">
          <template v-if="(scope.row.State === weiwStuffReturnBasicState.Abandon || scope.row.State === weiwStuffReturnBasicState.Reject || scope.row.State === weiwStuffReturnBasicState.Cancel)&&scope.row.CheckNote">
            <el-tooltip class="item" :content="scope.row.CheckNote" placement="top" effect="light">
              <span :class="scope.row.State | findKey(weiwStuffReturnBasicState)">{{weiwStuffReturnBasicState.Types[scope.row.State]}}</span>
            </el-tooltip>
          </template>
          <template v-else>
            <span :class="scope.row.State | findKey(weiwStuffReturnBasicState)">{{weiwStuffReturnBasicState.Types[scope.row.State]}}</span>
      ...
      <el-table-column label="操作" min-width="140" fixed="right">
        <template slot-scope="scope">
          <router-link :to="{path: '/depot/outSReturn/check',query: {id: scope.row.ReturnId}}" class="btn-link el-button el-button--text">查看</router-link>
          <template v-if="scope.row.State == weiwStuffReturnBasicState.Draft || scope.row.State == weiwStuffReturnBasicState.Reject">
            <router-link :to="{path: '/depot/outSReturn/edit', query: {id:scope.row.ReturnId}}" class="btn-link el-button el-button--text">编辑</router-link>
            <el-button type="text" @click="openDialog(scope.row, 'abandonDialog')">作废</el-button>
          </template>
          <el-button v-if="scope.row.State == weiwStuffReturnBasicState.Wait" type="text" @click="openDialog(scope.row, 'auditDialog')">审核</el-button>
          <el-button v-if="scope.row.State == weiwStuffReturnBasicState.Audit" type="text" @click="openDialog(scope.row, 'cancelDialog')">取消审核</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- End 数据表格 -->
      ...
      <el-form :model="createForm" label-position="right" label-width="100px" :rules="rules" ref="createForm">
        ...
        <el-form-item label="退料原因：" prop="ReasonTypeDk">
          <el-select v-model="createForm.ReasonTypeDk" placeholder="请选择" :filterable="true">
            ...
            <el-option v-for="(item, index) in reasonData.filter(item => {return item.State == YNStatus.Yes})" :key="index" :label="item.Value" :value="item.Id"></el-option>
          </el-select>
    ...
    <dictManage v-if="dictDialog" :visible.sync="dictDialog" @listenDictSave="getReasons" :dialogTitle="dialogTitle" :dictType="settingDictionaryDictType.WeiwStuffReturnBasicReasonType" :dicts="reasonData"></dictManage>
...
import {
  STOCKING_API_WEIW_STUFF_RETURN_BASIC_GETS,
  STOCKING_API_WEIW_STUFF_RETURN_BASIC_CREATE
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      STOCKING_API_WEIW_STUFF_RETURN_BASIC_GETS(this.queryForm).then(res => {
    ...
    multiAudit() {
      if (
        ...
        this.selections.some(item => item.State != this.weiwStuffReturnBasicState.Wait)
      ) {
    ...
    onSearch() {
      ...
      console.log('this.parameters', this.parameters, 'this.queryForm', this.queryForm)
    ...
    getReasons(){
      // this.$store.dispatch('GET_REASONS_DROPLIST', {DictType: this.settingDictionaryDictType.WeiwStuffReturnBasicReasonType,State: 0})
      MERCHANT_API_DROPDOWN_SETTINGDICTIONARYLIST({
