...
import {
  STOCKING_API_GOODS_COUNT_REPORT_BASIC_REJECTS2,
  STOCKING_API_GOODS_COUNT_REPORT_BASIC_REJECT2,
  STOCKING_API_GOODS_COUNT_REPORT_BASIC_AUDITS2,
  STOCKING_API_GOODS_COUNT_REPORT_BASIC_AUDIT2
    ...
    audit() {
      ...
      if (this.data.length > 1) {
        if (this.auditType === YNStatus.No) {
          result = STOCKING_API_GOODS_COUNT_REPORT_BASIC_REJECTS2({
          ...
          result = STOCKING_API_GOODS_COUNT_REPORT_BASIC_AUDITS2({
            Items: this.data.map(item => {
              return {
                ReportId: item.ReportId
              }
        ...
        if (this.auditType === YNStatus.No) {
          result = STOCKING_API_GOODS_COUNT_REPORT_BASIC_REJECT2({
            CheckNote: this.auditReson,
            ReportId: this.data[0].ReportId
          })
        } else {
          result = STOCKING_API_GOODS_COUNT_REPORT_BASIC_AUDIT2({
            ReportId: this.data[0].ReportId
          })
        }
      }
