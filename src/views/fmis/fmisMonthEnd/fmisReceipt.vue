    ...
    <el-form :model="queryForm" label-position="right" label-width="100px" :inline="true" class="item-lh-26 p-10" ref="search">
      <el-row class="search-box no-border" type="flex">
        <el-col :span="18">
          ...
          <el-form-item prop="ObjectType" label="对象类型：">
    ...
    <el-table :data="data" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中" @sort-change="sortChange">
      <el-table-column prop="PaidCode" label="单据编号" min-width="160" sortable="custom" show-overflow-tooltip fixed>
        <template slot-scope="scope">
          <el-button type="text" name="btnLinkPreviousType" @click="$router.push({path: '/fmis/receiptorder/receiptorderCheck', query: {id: scope.row.PaidId}})">{{scope.row.PaidCode}}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="PaymentTypeEv" label="收款方式" min-width="100" show-overflow-tooltip></el-table-column>
      <el-table-column prop="BillCode" label="来源单号" min-width="100" show-overflow-tooltip>
        <template slot-scope="scope">
          <el-button type="text" name="btnLinkPreviousType" @click="$router.push({path: '/fmis/receipt/receiptCheck', query: {id: scope.row.BillId}})">{{scope.row.BillCode}}</el-button>
        </template>
      </el-table-column>
