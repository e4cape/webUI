<template>
  <div class="content">
    <el-form :model="queryForm" ref="search" lable-width="120px" class="item-lh-26" :inline="true">
      <search-panel name="btnReceiptForm" @onSearch="onSearch" @onReset="onReset">
        ...
        <template slot="simpleSearch">
          <el-form-item>
            <el-select name="paidState" v-model="queryForm.PaidState" placeholder="所有状态" :filterable="true" @change="onSearch">
              ...
              <el-option v-for="(item,index) in settleIOBillBasicPaidStates.Types" :key="index" :label="item + '收款'" :value="index"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item prop="BillCode">
            <el-input name="billCode" v-model="queryForm.BillCode" placeholder="单据编号" @keyup.enter.native="onSearch">
              <el-button name="btnReceiptConfirm" slot="append" icon="el-icon-search" @click="onSearch"></el-button>
            </el-input>
          </el-form-item>
        </template>
        <template slot="seniorSearch">
          ...
          <el-form-item prop="ReasonTypeDk" label="所有原因：">
            <el-select name="ReasonTypeDk" v-model="queryForm.ReasonTypeDk" placeholder="所有原因">
              ...
              <el-option v-for="(item,index) in receiptReasons" :key="index" :label="item.Value" :value="item.Id"></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item prop="ObjectNote" label="应收对象：">
    ...
    <el-table :data="data" @sort-change="sortChange" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中">
      ...
      <el-table-column prop="ObjectNote" label="应收对象" min-width="100" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column prop="ReasonTypeDv" label="收款原因" min-width="100" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column prop="PreviousCode" label="来源单号" min-width="140" show-overflow-tooltip>
        <template slot-scope="scope">
          ...
          <router-link v-else class="text-blue" name="btnLinkPreviousType" :to="{path: previousTypeLink[scope.row.PreviousType], query: {id: scope.row.PreviousId}}">{{scope.row.PreviousCode}}</router-link>
        </template>
      </el-table-column>
      <el-table-column prop="BillPrice" label="应收金额" min-width="120" show-overflow-tooltip>
        <template slot-scope="scope">{{scope.row.BillPrice | initPrice}}</template>
      </el-table-column>
      <el-table-column prop="PaidPrice" label="已收金额" min-width="100" show-overflow-tooltip>
      ...
      <el-table-column prop="PaidState" label="状态" min-width="100" show-overflow-tooltip fixed="right">
        <template slot-scope="scope">{{settleIOBillBasicPaidStates.Types[scope.row.PaidState]}}收款</template>
      </el-table-column>
      <el-table-column label="操作" min-width="180" fixed="right">
        <template slot-scope="scope">
          <el-button @click="$router.push({path: '/fmis/receipt/receiptCheck', query: {id: scope.row.BillId}})" type="text">查看</el-button>
          <el-button @click="$router.push({path: '/fmis/receipt/receiptCreate', query: {id: scope.row.BillId}})" type="text" v-if="scope.row.PaidState != settleIOBillBasicPaidStates.All">创建收款单</el-button>
    ...
    <!-- @module Dialog·新建应收款单 -->
    <el-dialog title="新建应收款单" width="620px" :visible.sync="createVisible" @close="resetForm">
      <el-form label-position="right" label-width="100px" :model="createForm" :rules="rules" ref="createForm">
        ...
        <el-form-item label="应收对象" prop="ObjectType">
        ...
        <el-form-item label="对象名称" prop="PartnerId" v-if="createForm.ObjectType == settleIOBillBasicObjectTypes.Partner">
          <el-select name="partnerId" v-model="createForm.PartnerId" placeholder="请选择">
            ...
            <el-option :key="index" :label="item.Value" :value="item.Id" v-if="item.State == EnableState.Enable"></el-option>
            </template>
          </el-select>
        </el-form-item>
        <el-form-item label="对象名称" prop="StorechterId" v-if="createForm.ObjectType == settleIOBillBasicObjectTypes.Store">
          <el-select name="storechterId" v-model="createForm.StorechterId" placeholder="请选择">
            <template v-for="(item,index) in $store.getters.stores">
              <el-option :key="index" v-if="item.CharacterId && item.CharacterId != $store.getters.user_session.CharacterId" :label="item.Value" :value="item.CharacterId"></el-option>
            </template>
          </el-select>
        </el-form-item>
          ...
          <el-row>
            <el-col :span="14">
              <el-input :maxlength="50" placeholder="输入/扫描会员" @keyup.enter.native="searchVip" v-model="memberSN" name="memberSN">
                <el-button slot="append" icon="el-icon-search" name="btnSearchMember" @click="vipDialog = true"></el-button>
              </el-input>
            </el-col>
            <el-col :span="10">
        ...
        <el-form-item label="收款原因" prop="ReasonTypeDk">
        ...
        <el-form-item label="应收金额：" prop="BillPrice">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="createReceipt" :loading="$store.getters.is_loading" name="btnCreateReturn">确 定</el-button>
        <el-button @click="createVisible = false" name="btnCancel">取 消</el-button>
      </span>
    </el-dialog>
    <!-- End Dialog·新建应收款单 -->

    <!-- @module Dialog·新建收款原因 -->
    <dict-manage
      v-if="reasonDialog"
      :visible.sync="reasonDialog"
      :dicts="receiptReasons"
      dialogTitle="收款原因"
      :dictType="settingDictionaryDictTypes.SettleIOBillBasicReceiveReasonType"
      @listenDictSave="getReceiptReasons"
    ></dict-manage>
    <!-- End Dialog·新建收款原因  -->
...
export default {
  data() {
    return {
      ...
      queryForm: {
        // 查询表单数据
        BillType: SettleIOBillBasicBillType.Receipt,
        PreviousType: '0',
        ObjectType: '0',
        ReasonTypeDk: '0',
        ObjectNote: '',
  ...
  methods: {
      ...
      this.$refs['createForm'].validate(valid => {
        if (valid) {
          let param = Object.assign(
            {
              ReasonTypeDv: this.receiptReasons.find(
          ...
          param.BillPrice = this.$root.toInt(param.BillPrice)
    ...
    getReceiptReasons() {
      MERCHANT_API_DROPDOWN_SETTINGDICTIONARYLIST({
        DictType: SettingDictionaryDictType.SettleIOBillBasicReceiveReasonType,
        State: EnableState.Enable
      }).then(res => {
        if (res.data.Code === 'CORRECT') {
          this.receiptReasons = res.data.Data.Rows || []
        }
      })
    },
    searchVip() {
