<template>
  <div class="content">
        ...
        <div class="p-10">
          ...
          <el-form :model="createForm" label-width="110px" class="p10 init-form" :rules="rules" ref="createForm">
            <el-form-item label="收款金额：" prop="PaidPrice">
              <el-input name="btnReceiptCreateAmount" v-model="createForm.PaidPrice" @keyup.native="createForm.PaidPrice = $root.toFixed(createForm.PaidPrice, 2, true)"></el-input>
            </el-form-item>
            <el-form-item label="收款账户：" prop="BankTypeDk">
              <el-select name="btnReceiptCreateAccNum" v-model="createForm.BankTypeDk" placeholder="请选择收款账户" :filterable="true">
            ...
            <el-form-item label="收款方式：" prop="PaymentTypeEk">
              <el-select name="btnSelectReceiptTheWay" v-model="createForm.PaymentTypeEk" placeholder="请选择收款方式" :filterable="true">
    ...
    <!-- End panel -->
    <div class="buttons">
      <el-button type="primary" name="btnReceiptSave" @click="createReceipt" :loading="$store.getters.is_loading">保存</el-button>
      <router-link name="btnLinkReceiptIndex" :to="{path:'/fmis/receipt/index'}">
        <el-button name="btnCancel">取消</el-button>
      </router-link>
    </div>
    <!-- @module Dialog·新建收款账户 -->
    <dict-manage
      v-if="surnameDialog"
        ...
        PaidPrice: [
          {
            ...
            validator: (rule, value, callback) => {
              if (
                ...
                callback(new Error('最大值不能超过应收金额'))
              }
              callback()
            }
          }
