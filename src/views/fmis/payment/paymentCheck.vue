<template>
  <div class="content">
    ...
    <div class="panel">
      ...
      <div class="panel-bd">
            ...
            <tbody>
              <tr>
                <td rowspan="4" class="state-img">
                  ...
                  <div>{{settleIOBillBasicPaidState.Types[detail.PaidState] + '付款'}}</div>
                </td>
              </tr>
              <tr>
                <td class="tit">单号：</td>
        ...
        <el-table :data="data" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中">
          <el-table-column prop="PaidCode" label="付款单号" min-width="120" show-overflow-tooltip></el-table-column>
          ...
          <el-table-column prop="State" label="状态" min-width="100" show-overflow-tooltip>
            <template slot-scope="scope">{{settleIOBillPaidState.Types[scope.row.State]}}</template>
          </el-table-column>
    ...
    <!-- End panel -->
    <div class="buttons">      
      <el-button @click="$router.push({path: '/fmis/payment/paymentCreate', query: {id: detail.BillId}})" v-if="detail.PaidState !== settleIOBillBasicPaidState.All">创建付款单</el-button>
      ...
      <el-button name="btnPaymentCheckPrint" @click="printDialog = true">打印</el-button>
    </div>
    ...
    <print-order
      ...
      :printingType="settingPrintingTypes.StockingCloudSettleIOBillBasic3"
    ></print-order>
    <!-- End Dialog·打印 -->
  </div>
</template>
      ...
      this.$confirm('确定将此单据删除?', '提示', {
        ...
        STOCKING_API_SETTLE_IO_BILL_BASIC_ABANDON({ BillId: this.billId }).then(res => {
          ...
          if (res.data.Code === 'CORRECT') {
            ...
            this.$router.replace({ path: '/fmis/payment/index' })
          }
        })
      })
    },
