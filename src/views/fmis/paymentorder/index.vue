<template>
  <div class="content">
    <el-form :model="queryForm" ref="search" name="btnPaidOrderForm" lable-width="120px" class="item-lh-26" :inline="true">
      <search-panel name="btnReceoptOrderSearch" @onSearch="onSearch" @onReset="onReset">
        <template slot="btnBox">
          <el-dropdown name="btnPaidOrderBatch" @command="multiOperation" v-if="data.length">
        ...
        <template slot="seniorSearch">
          ...
          <el-form-item prop="BankTypeDk" label="付款账户：">
          ...
          <el-form-item prop="PaymentTypeEk" label="付款方式：">
    ...
    <el-table :data="data" @sort-change="sortChange" @selection-change="selectionChange" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中">
      ...
      <el-table-column prop="BillCode" label="来源单号" min-width="140" show-overflow-tooltip>
        <template slot-scope="scope">
          <router-link class="text-blue" name="btnLinkPreviousType" :to="{path: '/fmis/payment/paymentCheck', query: {id: scope.row.BillId}}">{{scope.row.BillCode}}</router-link>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="PaymentTypeEk" label="付款方式" min-width="100" show-overflow-tooltip>
      ...
      <el-table-column prop="State" label="状态" min-width="100" show-overflow-tooltip fixed="right">
        <template slot-scope="scope">
          <span :class="{'gray9':scope.row.State === (settleIOBillPaidStates.Abandon || settleIOBillPaidStates.Cancel)}">{{settleIOBillPaidStates.Types[scope.row.State]}}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" min-width="150" fixed="right">
        <template slot-scope="scope">
          <el-button type="text" name="btnPaidOrderCheck" @click="$router.push({path: '/fmis/paymentorder/paymentorderCheck', query: {id: scope.row.PaidId}})">查看</el-button>
