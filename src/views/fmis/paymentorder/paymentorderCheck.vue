<template>
  <div class="content">
    ...
    <print-order
      ...
      :printingType="settingPrintingType.StockingCloudSettleIOBillPaid3"
      ...
      this.$confirm('确定作废所选付款单', '提示', {
        ...
        .then(() => {
          STOCKING_API_SETTLE_IO_BILL_PAID_ABANDONS({
            Items: [this.$route.query.id]
          }).then(res => {
            if (res.data.Code === 'CORRECT') {
              this.$message.success('提交成功')
              this.$router.replace({path: '/fmis/paymentorder/index'})
            }
          })
        })
    },
