<template>
  <div>
    <el-form :model="queryForm" label-width="100px" ref="queryForm" :inline="true" class="item-lh-26 p10">
      <el-row class="search-box no-border" type="flex">
        <el-col>
          ...
          <el-form-item label="归属公司编码：" prop="sourceCode" v-if="$store.getters.user_session.CharacterType == characterTypes.Lingcb">
            <el-input name="sourceCode" v-model="queryForm.sourceCode" @keyup.enter.native="onSearch"></el-input>
          </el-form-item>
          <el-form-item label="归属公司名称：" prop="storeName" v-if="$store.getters.user_session.CharacterType == characterTypes.Lingcb">
            <el-input name="btnAttribttion" v-model="queryForm.storeName" @keyup.enter.native="onSearch"></el-input>
          </el-form-item>
        </el-col>
    ...
    <el-table :data="data" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中">
      ...
      <el-table-column prop="sourceCode" label="归属公司编码" width="100" show-overflow-tooltip v-if="$store.getters.user_session.CharacterType == characterTypes.Lingcb"></el-table-column>
      <el-table-column prop="storeName" label="归属公司名称" width="100" show-overflow-tooltip v-if="$store.getters.user_session.CharacterType == characterTypes.Lingcb"></el-table-column>
      ...
      <el-table-column label="操作" width="90" fixed="right">
        <template slot-scope="scope">
          <el-button name="btnDeleteTemplate" type="text" @click="deleteTemplate(scope.row)" v-if="scope.row.isStatic == 0 && $store.getters.user_session.CharacterType == characterTypes.Lingcb">删除</el-button>
          <el-switch
...
import {
  MESSAGE_API_SETTINGTEMPLATE_SEARCHCOMPANYTEMPLATES,
