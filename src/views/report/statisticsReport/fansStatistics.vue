...
import {
  MEMBERSHIP_API_REPORT_FANSSTATISTICS
...
export default {
  ...
  methods: {
    ...
    fansStatistics() {
      ...
      MEMBERSHIP_API_REPORT_FANSSTATISTICS(param).then(res => {
        if (res.data.Code === 'CORRECT') {
          ...
          for (let i = 0; i < chartDataArr.length; i += 1) {
            ...
            newfansCountArr.push(chartDataArr[i].fansCount)
            lostCountArr.push(chartDataArr[i].lostCount)
            changeCountArr.push(chartDataArr[i].changeCount)
          }
          this.chartOptions.xAxis.data = xAxisDataArr
          this.chartOptions.series[0].data = newfansCountArr
          this.chartOptions.series[1].data = lostCountArr
          this.chartOptions.series[2].data = changeCountArr
        }
