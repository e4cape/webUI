    ...
    <el-form :model="formInline" ref="search" lable-width="120px" class="item-lh-26" :inline="true">
      <search-panel :isSenior="false">
        <template slot="btnBox">
          <el-form-item>
            <el-button name="btnbtn" type="primary">导出报表</el-button>
          </el-form-item>
        </template>
...
// import {
//   PERFORMANCE_API_REPORT_BONUSLIST
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      this.API_REPORT_BONUSEXPORT({
        Year: this.formInline.year, UserId: this.formInline.userId, isLoading: true
      }).then(res => {
    ...
    GetQuarter() {
      ...
      //   if (_data.length > 0) {
      //     _data.forEach(m => {
      //       report.series[0].data[parseInt(dayjs(new Date(m.EventDate)).format('M')) - 1] = m.BonusPrice.toFixed(2)
      //     })
      //   }
      //   this.polar = report
      //   let ary = {
