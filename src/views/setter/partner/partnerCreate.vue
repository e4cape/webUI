<template>
  <el-dialog title="新建单位" :visible.sync="visible" custom-class="supplier-dialog">
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="单位类型：" prop="PartnerType">
            <el-select filterable v-model="createForm.PartnerType" placeholder="请选择" name="PartnerType">
              <el-option v-for="(item,index) in partnerType.Types" :key="index" :label="item" :value="Number(index)"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item label="结算方式：" prop="SettleType">
            <el-radio-group v-model="createForm.SettleType">
              <el-radio v-for="(item, index) in partnerBasicSettleType.Types" :label="Number(index)" :key="index">{{item}}</el-radio>
            </el-radio-group>
          </el-form-item>
        </el-col>
      </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      areaData: [],
      createForm: {
        PartnerType: '', //	int32	往来单位类型(枚举)	required
        PartnerCode: '', //	string	往来单位编码	required,max=50
        PartnerName: '', //	string	往来单位名称	required,max=50
        ShortName: '', //	string	往来单位简称	max=50
        AuthorizeCode: '', //	string	授权码	max=50
        SettleType: '', //	int32	结算方式(枚举)	gte=0
  ...
  methods: {
    ...
    createData() {
      ...
      this.createForm.Taxes = this.createForm.Taxes || 0
      let param = Object.assign({}, this.createForm, {Taxes: this.$root.toInt(this.createForm.Taxes)})
