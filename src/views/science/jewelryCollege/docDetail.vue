    ...
    <div class="bd">
      ...
      <template v-if="basicInfo.State == EnumInfrastCourseState.Draft || basicInfo.State == EnumInfrastCourseState.Reject || basicInfo.State == EnumInfrastCourseState.Cancel">
        <el-button
          name="btnEdit"
          type="primary"
          @click="$router.push(`/science/jewelryCollege/docEdit?id=${form.CourseId}`)"
        >编辑</el-button>
        <el-button
          name="btnInvalid"
          @click="invalid"
...
<script>
import {
  COLLEGE_API_INFRASTCOURSEBASIC_COLLEGEDETAIL, // 系统详情
...
export default {
  ...
  methods: {
    ...
    initRoute() {
      this.$router.replace({
        path: `/science/jewelryCollege/docDetail?id=${this.$route.query.id}`,
