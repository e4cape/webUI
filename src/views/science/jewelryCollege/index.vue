...
<template>
  <div class="sys-training">
    ...
    >
      ...
      >
        <template slot="btnBox">
          <el-form-item>
            <el-button
              name="btnVisibleDocVideoModal"
              type="primary"
              @click="docVideoNew(EnumInfrastCourseType.Article)"
            >新建文档</el-button>
          </el-form-item>
        </template>
        <template slot="btnBox">
          <el-form-item>
            <el-button
              name="btnVisibleDocVideoModal"
              type="primary"
              @click="docVideoNew(EnumInfrastCourseType.Video)"
            >新建视频</el-button>
          </el-form-item>
        </template>
        <template slot="simpleSearch">
          <el-form-item>
            ...
            >
              ...
              ></el-option>
              <el-option
                v-for="item in EnumInfrastCourseState.TypeArray"
                :key="item.KeyId"
                :label="item.Value"
                :value="item.KeyId"
              ></el-option>
        ...
        <template slot="seniorSearch">
          ...
          <el-form-item
            label="所属课程："
            prop="collegeArrSelect"
          >
            <el-cascader
              name="selectLargeId"
              :options="[allCategory, ...collegeArr]"
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            @click="$router.push(`/science/jewelryCollege/docDetail?id=${scope.row.CourseId}`)"
            type="text"
            size="small"
          >详情</el-button>
          <el-button
            name="btnLook"
            v-else
            @click="$router.push(`/science/jewelryCollege/videoDetail?id=${scope.row.CourseId}`)"
            type="text"
            size="small"
          >详情</el-button>
          <el-button
            name="btnAudit"
            v-if="scope.row.State == EnumInfrastCourseState.Wait"
            type="text"
            size="small"
          ...
          <template v-if="scope.row.State == EnumInfrastCourseState.Draft || scope.row.State == EnumInfrastCourseState.Reject || scope.row.State == EnumInfrastCourseState.Cancel">
            <el-button
              name="btnEditDoc"
              v-if="scope.row.CourseType == EnumInfrastCourseType.Article"
              @click="$router.push(`/science/jewelryCollege/docEdit?id=${scope.row.CourseId}`)"
              type="text"
              size="small"
            >编辑</el-button>
            <el-button
              name="btnEdiVideo"
              v-else
              @click="$router.push(`/science/jewelryCollege/videoEdit?id=${scope.row.CourseId}`)"
              type="text"
              size="small"
            >编辑</el-button>
            <el-button
    ...
    ></pagination>
    <docVideoModal
      v-if="visibleDocVideoModal"
      :visibleDocVideoModal="visibleDocVideoModal"
...
import {
  ...
  COLLEGE_API_SETTINGDICTIONARY_DROPDOWNLISTBYCOLLEGE, // 课程分类-下拉框
...
export default {
  ...
  methods: {
    ...
    // 所属课程change
    collegeArrChange(v) {
      this.form.LargeId = v[0]
      this.form.SmallId = v[1]
    },
    // 表格分页相关
    async init() {
      ...
      if (query.LargeId > 0) {
        this.collegeArrSelect = [+query.LargeId, +query.SmallId || 0]
      }
      this.getData()
    },
    initRoute() {
    ...
    getData() {
      ...
      COLLEGE_API_INFRASTCOURSEBASIC_COLLEGELISTBYLCB(this.form).then(res => {
        if (res.data.Code == 'CORRECT') {
    ...
    listenVisibleDocVideoModal(succ, id) {
      if (succ) {
        if (this.courseType === InfrastCourseType.Article) {
          this.$router.push(
            `/science/jewelryCollege/docEdit?id=${id}`
          )
        } else {
          this.$router.push(
            `/science/jewelryCollege/videoEdit?id=${id}`
          )
        }
      }
    ...
    invalid(row) {
      this.title = '作废'
      this.apiName = 'COLLEGE_API_INFRASTCOURSEBASIC_ABANDONCOLLEGE'
      this.invalidCancelObj = row
      this.visibleInvalidCancelModal = true
    },
    auditCancel(row) {
      this.title = '取消审核'
      this.apiName = 'COLLEGE_API_INFRASTCOURSEBASIC_CANCELCOLLEGE'
      this.invalidCancelObj = row
      this.visibleInvalidCancelModal = true
    },
