...
import {
  ...
  COLLEGE_API_INFRASTCOURSEBASIC_MODIFYBYCOLLEGEVIDEO, // 更新视频内容
...
export default {
  ...
  methods: {
    ...
    initRoute() {
      this.$router.replace({
        path: `/science/jewelryCollege/videoEdit?id=${this.$route.query.id}`,
    ...
    saveConForm() {
      ...
      this.$refs.conForm.validate(valid => {
        if (valid) {
          ...
          COLLEGE_API_INFRASTCOURSEBASIC_MODIFYBYCOLLEGEVIDEO(param)
            .then(res => {
              if (res.data.Code === 'CORRECT') {
                this.getInfrastCourseBasic()
              }
