<template>
  <div class="sys-training-detail">
    <basicInfoTb :basicInfo="basicInfo" :loadingBasic="loadingBasic" :channelType="EnumInfrastCourseChannelType.College">
      <el-button name="btnVisibleDocModal" type="text" @click="visibleDocModal = true">修改</el-button>
    </basicInfoTb>
    <div class="md clear">
      <el-tabs type="card" v-model="parameter.activeName" @tab-click="tabClick" class="lcb-tabs">
        <el-tab-pane label="内容" name="first">
          <el-form :model="conForm" :rules="rules" ref="conForm" label-width="60px" class="con-form">
            ...
            <el-form-item label="简介：" prop="CourseNote">
              <el-col :span="14">
                <el-input type="textarea" :rows="4" v-model="conForm.CourseNote" maxlength="300"></el-input>
                <span class="note word" v-if="conForm.CourseNote.length">{{conForm.CourseNote.length}}/300字</span>
              </el-col>
            </el-form-item>
            <el-form-item label="正文：" prop="Content">
              <el-col :span="14">
                <editor @listenEditerChange="listenEditerChange" :Note="html"></editor>
                <span v-if="editorText.length" class="note word">
                  <i :class="editorText.length>2000?'red':''">{{editorText.length}}</i>/2000字
                </span>
              </el-col>
            </el-form-item>
            <el-form-item>
              <el-button type="primary" @click="saveConForm" :loading="saveConFormLoading">保存</el-button>
            </el-form-item>
          </el-form>
        </el-tab-pane>
        <el-tab-pane v-if="basicInfo.IsPaper == EnumYNStatus.Yes" label="题库" name="second">
          <div class="question">
            <div class="about clearfix">
              <div class="fl">
                <el-button type="primary" @click="addSubject">增加题目</el-button>实际考试时系统随机选题，选项打乱顺序。最多可添加200道题。以下正确答案黄色加粗显示。
              </div>
              <div class="fr">
                <div class="fl">
            ...
            <el-table :data="tableData" v-loading="$store.getters.tb_loading">
              <el-table-column label="序号" prop="QuesId" min-width="50" fixed="left" show-overflow-tooltip></el-table-column>
              <el-table-column label="题目" prop="Title" min-width="200" fixed="left" show-overflow-tooltip></el-table-column>
              <el-table-column label="图片" prop="ImageUrl" min-width="200" show-overflow-tooltip>
                <template slot-scope="scope">
                  <img v-if="scope.row.ImageUrl" :src="$root.settings.DOMAIN_IMG_FILE+scope.row.ImageUrl" alt>
                </template>
              </el-table-column>
              <el-table-column label="题目类型" prop="joinTime" min-width="100" show-overflow-tooltip>
                <template slot-scope="scope">{{EnumInfrastCourseQuesType.Types[scope.row.QuesType]}}</template>
              </el-table-column>
              <el-table-column :label="`选项${k+1}`" prop="Options" min-width="100" show-overflow-tooltip v-for="(v,k) in 6" :key="k">
                <template slot-scope="scope">
              ...
              <el-table-column label="操作" width="100" fixed="right">
                <template slot-scope="scope">
                  <el-button name="btnLook" @click="del($event,scope.row.QuesId)" type="text" size="small">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
            <pagination :total="total" :pg="form.PageIndex" :size="form.PageSize" @currentChange="currentChange" @sizeChange="sizeChange"></pagination>
            <div v-if="parameter.activeName == 'second' && tableData.length<1" style="height:10px"></div>
          </div>
        </el-tab-pane>
      </el-tabs>
    </div>
    ...
    <docVideoModal
      ...
      title="修改文档(基本信息)"
      :channelType="EnumInfrastCourseChannelType.College"
...
<script>
import {
  COLLEGE_API_INFRASTCOURSEBASIC_COLLEGEDETAIL, // 学院详情
  COLLEGE_API_INFRASTCOURSEBASIC_MODIFYBYCOLLEGEARTICLE, // 更新文档内容
  COLLEGE_API_INFRASTCOURSEQUES_COLLEGELIST, // 题库列表
  COLLEGE_API_INFRASTCOURSEQUES_DELETEBYCOLLEGE, // 题库删除
  COLLEGE_API_INFRASTCOURSEBASIC_SUBMITBYCOLLEGE // 提交审核
} from '@/apis/science'
...
export default {
  ...
  methods: {
    ...
    getInfrastCourseBasic() {
      ...
      COLLEGE_API_INFRASTCOURSEBASIC_COLLEGEDETAIL({
        CourseId: this.$route.query.id
      })
        .then(res => {
    ...
    initRoute() {
      this.$router.replace({
        path: `/science/jewelryCollege/docEdit?id=${this.$route.query.id}`,
    ...
    getData() {
      ...
      COLLEGE_API_INFRASTCOURSEQUES_COLLEGELIST(this.form).then(res => {
        if (res.data.Code == 'CORRECT') {
    ...
    del(e, QuesId) {
      ...
      this.$confirm('您正在进行删除操作，删除后不可恢复！确定删除?', '删除', {
        ...
        COLLEGE_API_INFRASTCOURSEQUES_DELETEBYCOLLEGE({ QuesId: QuesId }).then(res => {
          if (res.data.Code === 'CORRECT') {
    ...
    saveConForm() {
      ...
      this.$refs.conForm.validate(valid => {
        if (valid) {
          ...
          COLLEGE_API_INFRASTCOURSEBASIC_MODIFYBYCOLLEGEARTICLE(param)
            .then(res => {
              if (res.data.Code === 'CORRECT') {
                this.getInfrastCourseBasic()
              }
    ...
    submitAudit() {
      ...
      COLLEGE_API_INFRASTCOURSEBASIC_SUBMITBYCOLLEGE({ CourseId: this.$route.query.id }).then(
        res => {
          if (res.data.Code === 'CORRECT') {
            this.$router.push('/science/jewelryCollege/index')
          }
