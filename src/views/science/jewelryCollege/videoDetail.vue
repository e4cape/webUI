            ...
            <li>
              ...
              <div class="con-r">
                <div
                  ...
                  v-if="videoOption.playauth"
                >
                  <aliPlayer
                    :vid="videoOption.vid"
                    :playauth="videoOption.playauth"
                    :cover="videoOption.coverUrl"
                    width="640px"
                    height="360px"
                  ></aliPlayer>
                </div>
    ...
    <div class="bd">
      ...
      <template v-if="basicInfo.State == EnumInfrastCourseState.Draft || basicInfo.State == EnumInfrastCourseState.Reject || basicInfo.State == EnumInfrastCourseState.Cancel">
        <el-button
          name="btnEdit"
          type="primary"
          @click="$router.push(`/science/jewelryCollege/videoEdit?id=${form.CourseId}`)"
        >编辑</el-button>
        <el-button
          name="btnInvalid"
          @click="invalid"
...
import {
  COMMON_API_VOD_PLAY, // 获取(播放凭证)
  COMMON_API_VOD_GET // 获得封面
} from '@/apis/common'
import {
...
export default {
  ...
  methods: {
    ...
    getCoverUrl(videoId) {
      COMMON_API_VOD_GET({
        ...
        if (resp.data.Code == 'CORRECT') {
          this.videoOption.coverUrl = resp.data.Data.CoverURL
        }
      })
    },
    // 获取学院详情
      ...
      COLLEGE_API_INFRASTCOURSEBASIC_COLLEGEDETAIL({
        ...
        .then(res => {
          if (res.data.Code == 'CORRECT') {
            ...
            if (res.data.Data.VideoCode) {
              COMMON_API_VOD_PLAY({ VideoId: res.data.Data.VideoCode })
                .then(response => {
                  if (res.data.Code == 'CORRECT') {
                    this.videoOption.playauth = response.data.Data.PlayAuth
                    this.videoOption.vid = response.data.Data.VedioId
                    this.getCoverUrl(res.data.Data.VideoCode)
                  }
    ...
    initRoute() {
      this.$router.replace({
        path: `/science/jewelryCollege/videoDetail?id=${this.$route.query.id}`,
