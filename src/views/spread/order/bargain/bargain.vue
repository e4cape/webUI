    ...
    <el-form :model="queryForm" ref="search" lable-width="120px" class="item-lh-26" :inline="true">
      <search-panel @onSearch="onSearch" @onReset="onReset">
        <template slot="btnBox">
          <el-form-item>
            <el-button name="btnAllImport" type="primary" @click="allImport">导出Excel</el-button>
          </el-form-item>
        </template>
        <template slot="simpleSearch">
          <el-form-item>
            <el-input name="ProductId" v-model="queryForm.ProductId">
              <el-button name="btnSearch" slot="append" class="el-icon-search" placeholder="商品编码" @click="onSearch"></el-button>
            </el-input>
          </el-form-item>
        </template>
        <template slot="seniorSearch">
          ...
          <el-form-item label="商品编码：">
            <el-input name="ProductId" :maxlength="50" v-model="queryForm.ProductId" @keyup.native="queryForm.ProductId = $root.toFixed(queryForm.ProductId, 0)" @keyup.enter.native="onSearch"></el-input>
          </el-form-item>
          <el-form-item label="订单类型：">
            <el-select name="IsDirected" v-model="queryForm.IsDirected" placeholder="全部" :filterable="true">
              ...
              <el-option label="砍价" :value="yNStatus.No"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="订单号：">
            <el-input name="OrderCode" :maxlength="20" v-model="queryForm.OrderCode" @keyup.enter.native="onSearch"></el-input>
          </el-form-item>
          <el-form-item label="商品名称：">
            <el-input name="ProductName" :maxlength="20" v-model="queryForm.ProductName" @keyup.enter.native="onSearch"></el-input>
          </el-form-item>
          <el-form-item label="姓名：">
            <el-input name="TrueName" :maxlength="20" v-model="queryForm.TrueName" @keyup.enter.native="onSearch"></el-input>
          </el-form-item>
          <el-form-item label="手机号：">
            <el-input name="Mobile" :maxlength="20" v-model="queryForm.Mobile" @keyup.enter.native="onSearch"></el-input>
          </el-form-item>
        </template>
      </search-panel>
    </el-form>
...
import {
  SpreadSaleOrderBasicState, SpreadType, SaleOrderBasicReturnState, ExpressType, PickType, SpreadSaleOrderBasicReturnState, ShippingType} from '@/enums/spread'
import { CompanyBasicWechatSettingType } from '@/enums/membership'
import { YNStatus, CharacterType } from '@/enums/common'
import {
  SPREAD_API_SPRORDER_SEARCH, SPREAD_API_SPRORDER_EXPORT, SPREAD_API_SPRORDER_RETURNCREATE, SPREAD_API_SPRORDER_SEARCHREASON, SPREAD_API_SPRORDER_CHECKCOUPON
} from '@/apis/spread'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    check(row) {
      ...
      this.returnId =
        row.ReturnState === SaleOrderBasicReturnState.No
          ? false
          : row.ReturnId
    ...
    initRoute() {
      this.$router.replace({
        path: this.$router.path, query: Object.assign(this.parameters, {
          spreadId: this.spreadId
        })
      })
    }
