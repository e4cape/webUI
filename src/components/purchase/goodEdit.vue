    ...
    <div class="panel goodAdd-warpper">
      ...
      <div class="panel-bd" v-loading="$store.getters.tb_loading" element-loading-text="拼命加载中">
        <el-form :model="form" ref="form" class="purchase-form" :rules="rules" label-width="150px">
          ...
          <el-row :gutter="10">
            <el-col :span="18">
              <el-col :span="8" v-if="source === 'MODIFY'">
                <el-form-item label="货品类型：" >
                  <el-select filterable v-model="form.GoodsType" placeholder="请选择" :disabled="isDisable">
                    <el-option label="请选择" :value="0"></el-option>
                    <template v-for="(item2, index2) in enums.GoodsType.TypeArray">
                      <el-option
                        :label="item2.Value"
                        :value="item2.KeyId"
                        :key="index2"
                ...
                <el-form-item :prop="item.FieldEnName" :label="item.FieldCnName + '：'">
                  ...
                  >
                    <span v-if="item.FieldEnName.indexOf('Image') > -1">
                      <span v-if="form[item.FieldEnName] == ''" class="el-button--text">上传图片</span>
                      <uploadImg
                    ...
                    <span v-else-if="source === 'MODIFY' && (item.FieldEnName === 'BarCode' || (item.FieldEnName === 'StyleCode' && StyleId > 0))">{{forms[item.FieldEnName]}}</span>
                    <el-input
                      v-else
                      v-model="forms[item.FieldEnName]"
                      @blur="form[item.FieldEnName] = form[item.FieldEnName].trim()"
                      :maxlength="50"
                      @keyup.native="forms[item.FieldEnName] = forms[item.FieldEnName]"
                    ></el-input>
                  </template>
                  <template
                  ...
                  >
                    <span v-if="source === 'MODIFY' && item.FieldEnName === 'Quantity'">{{forms[item.FieldEnName]}}</span>
                    <el-input
                      v-else
                  ...
                  >
                    <el-input
                      ...
                      @blur="form[item.FieldEnName] = form[item.FieldEnName].trim()"
                      :maxlength="item.FieldType === enums.SettingCustomizedFieldType.TextOfInteger ? 8 : item.FieldType === enums.SettingCustomizedFieldType.TextOfDecimal ? 10 : 50"
                      @keyup.native="forms[item.FieldEnName] = $root.toFixed(forms[item.FieldEnName], item.Precision)"
                    ></el-input>
                  </template>
                  <template v-else>
                    <el-select
                      v-model="forms[item.FieldEnName]"
                      :prop="forms[item.FieldEnName]"
                      placeholder="请选择"
                      filterable
            ...
            <el-col :span="6">
              <div class="img-uploader" @click="iClick">
                <img src="@/assets/images/upload.jpg" style="width: 100%;height: 100%;" v-if="form.ImageUrl == ''">
                <img
                  :src="$root.settings.DOMAIN_IMG_FILE + forms.ImageUrl"
                  alt
                  v-else
              ...
              <!-- <uploadImg
                :uploadImgLoading="ImgLoading"
                :Root="$root.filePaths.STOCKING_PURCHASE"
                :uploadImageUrl="$root.settings.DOMAIN_IMG_FILE + form.ImageUrl"
                :styleObj="{
                  width: '200px',
                  height: '200px',
                  lineHeight: '200px',
                ...
                @uploadSucc="(key) => {uploadSucc(key, form, 'ImageUrl')}"
              ></uploadImg>
              <div class="uploadeText">上传图片</div>-->
            </el-col>
          </el-row>
          <!-- End 货品属性表单 -->
...
export default {
  ...
  methods: {
    ...
    getForm() {
      this.isDisable = this.forms.GoodsType == GoodsType.Single ? false : true
    ...
    saveData() {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          this.allData.map(item => {
            if (item.Precision > 0 || item.FieldType == SettingCustomizedFieldType.TextOfPercent) {
    ...
    selectChange(val, Enums) {
      if(val.indexOf('Ek')) {
        ...
        this.form[name] = Enums.find(i => i.Value === this.form[val]) ? Enums.find(i => i.Value === this.form[val]).Title : ''
      }
    },
    listenCropper(files) {
  ...
  mounted() {
    this.getForm()
    this.isCanInput = this.forms.StyleCode ? true : false
  },
  beforeMount() {},
  watch: {}
}
