      ...
      >
        ...
        <template slot-scope="scope">
          <template v-if="item.FieldType === enums.SettingCustomizedFieldType.TextOfTextual">
            ...
            ></uploadImg>
            <el-input v-model="scope.row[item.FieldEnName]" v-else-if="scope.$index >= currentStartIndex && scope.$index <= currentEndIndex" :maxlength="50" @focus="handelFocus(scope.row)"></el-input>
          </template>
          <template
            ...
            >
              <template v-if="item.Enumerate && item.Enumerate != ''">
                <el-option
                  label="请选择"
                  :value="0"
                  v-if="item.IsRequired == enums.YNStatus.No"
                ></el-option>
...
export default {
  ...
  methods: {
    ...
    uploadSucc(Key, row, name) {
      ...
      this.isSaved = true
      this.$emit('changeSave', {isSaved: this.isSaved, saveLoading: this.saveLoading})
    },
    handelFocus(row) {
    ...
    handleCurrentChange(row, oldRow) {
      ...
      if (oldRow) {
        ...
        this.tableData.some(item => {
          if(item.IsRequired == this.enums.YNStatus.Yes && oldRow[item.FieldEnName] === '') {
            this.isRet = false
            return this.$message.error(`${item.FieldType === this.enums.SettingCustomizedFieldType.SelectOfEnums ? '请选择' : '请输入'}${item.FieldCnName}`)
          }
        })
        if (this.isRet) {
          ...
          this.tableData.forEach(item => {
            ...
            } else if (item.FieldType === this.enums.SettingCustomizedFieldType.TextOfInteger) {
              parameters[item.FieldEnName] = parseInt(parameters[item.FieldEnName]) || 0
            }
          })
      ...
      if (this.fieldData.length !== 0) {
        ...
        this.tableData.forEach(val => {
          this.goodsData.forEach(item => {
            if(val.Precision > 0 && item[val.FieldEnName] !== '') {
